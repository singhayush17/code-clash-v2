...
LESSONS = [
    ...
    Lesson("semi-anti-joins", 12, "Semi-Joins, Anti-Joins & EXISTS", ["EXISTS", "NOT EXISTS", "LEFT SEMI", "RIGHT SEMI"], ["customers", "orders", "products", "order_items", "shipments", "employees", "support_tickets"], [
            ...
            Task("l29-t9", "Right Semi Join using IN: Find employee IDs that have been assigned at least one support ticket. Use IN. Sort by id.", "", "SELECT id FROM employees WHERE id IN (SELECT employee_id FROM support_tickets WHERE employee_id IS NOT NULL) ORDER BY id;"),
            ...
            Task("l29-t11", "Left Anti Join: Find employee names who have never been assigned any support ticket. Use NOT EXISTS. Sort by name.", "", "SELECT name FROM employees WHERE NOT EXISTS (SELECT 1 FROM support_tickets WHERE support_tickets.employee_id = employees.id) ORDER BY name;"),
            ...
            Task("l29-t13", "Left Semi Join vs DISTINCT JOIN: Find customer names who have ordered 'MacBook Pro'. Use IN with a chained subquery (no JOINs in the outer query). Sort by name.", "", "SELECT name FROM customers WHERE id IN (SELECT customer_id FROM orders WHERE id IN (SELECT order_id FROM order_items WHERE product_id = (SELECT id FROM products WHERE name = 'MacBook Pro'))) ORDER BY name;"),
