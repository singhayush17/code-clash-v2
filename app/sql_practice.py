...
LESSONS = [
    ...
    Lesson("join-patterns", 13, "Join Interview Scenarios", ["identify the join"], ["customers", "orders", "products"], [
        ...
        Task("l10j-t5", "Scenario: 'Build a complete picture: Show every customer alongside any orders they may have, even if a customer never ordered.' Return customer name and order id, sorted by customer name then order id.", "", "SELECT customers.name, orders.id FROM customers LEFT JOIN orders ON orders.customer_id = customers.id ORDER BY customers.name, orders.id;"),
    ]),
    ...
    Lesson("mixed-bag-capstone", 57, "Mixed Bag: Ultimate Capstone", ["Hint-Free", "Advanced Business Logic",
