...
SEED: dict[str, list[tuple[Any, ...]]] = {
    ...
    "payments": [
        (201, 101, "card", 128.24, "success", "2024-01-05 09:00:00"),
        (202, 102, "wallet", 79.00, "failed", "2024-01-09 10:00:00"),
        (203, 102, "wallet", 79.00, "success", "2024-01-09 10:05:00"),
        (204, 103, "card", 149.00, "refunded", "2024-01-11 11:30:00"),
        (205, 104, "upi", 103.80, "success", "2024-01-09 14:00:00"),
        (206, 105, "card", 95.00, "failed", "2024-02-10 08:00:00"),
        (207, 105, "card", 95.00, "success", "2024-02-10 08:03:00"),
        (208, 106, "netbanking", 135.75, "success", "2024-02-14 12:00:00"),
        (209, 107, "wallet", 90.25, "failed", "2024-03-03 16:00:00"),
        (210, 107, "wallet", 90.25, "success", "2024-03-03 16:04:00"),
        (211, 108, "card", 174.99, "success", "2024-03-09 09:15:00"),
        (212, 109, "upi", 40.00, "failed", "2024-03-03 18:10:00"),
        (213, 109, "upi", 40.00, "failed", "2024-03-03 18:20:00"),
        (214, 109, "wallet", 40.00, "success", "2024-03-04 09:00:00"),
        (215, 110, "card", 185.00, "success", "2024-04-01 10:00:00"),
        (216, 110, "card", 185.00, "refunded", "2024-04-03 15:30:00"),
    ],
}
...
LESSONS = [
    ...
    Lesson("basic-capstone", 6, "Basic Queries: Interview Capstone", ["Hint-Free", "Filtering"], ["customers", "orders", "products"], [
        ...
        Task("bc-t2", "Find the cities of customers who live in 'India'.", "", "SELECT city FROM customers WHERE country = 'India';"),
        Task("bc-t3", "List all products that cost exactly 89.99.", "", "SELECT * FROM products WHERE price = 89.99;"),
        Task("bc-t4", "Identify orders that have a status of 'processing'. Return the order IDs.", "", "SELECT id FROM orders WHERE status = 'processing';"),
        ...
        Task("bc-t7", "Find all customers whose name starts with the letter 'M'. Return their names.", "", "SELECT name FROM customers WHERE name LIKE 'M%';"),
        ...
        Task("bc-t12", "List all products that belong to the 'Electronics' category and cost more than 100.", "", "SELECT * FROM products WHERE category = 'Electronics' AND price > 100;"),
        ...
        Task("bc-t14", "Identify products whose name contains the word 'Noise'. Return name.", "", "SELECT name FROM products WHERE name LIKE '%Noise%';"),
        ...
        Task("bc-t18", "Find products where the stock is dangerously low (less than 20) but not completely out. Return name and stock.", "", "SELECT name, stock FROM products WHERE stock > 0 AND stock < 20;"),
        Task("bc-t19", "Identify customers whose city name ends with 'ore'. Return name and city.", "", "SELECT name, city FROM customers WHERE city LIKE '%ore';"),
    ...
    Lesson("outer-joins", 8, "LEFT JOINs", ["LEFT JOIN", "missing rows"], ["products", "order_items", "orders", "shipments"], [
        ...
        Task("l7-t5", "Find customers who have never had a cancelled order. Show name, sorted by name.", "", "SELECT customers.name FROM customers LEFT JOIN orders ON orders.customer_id = customers.id AND orders.status = 'cancelled' WHERE orders.id IS NULL ORDER BY customers.name;"),
    ]),
    ...
    Lesson("full-joins", 10, "FULL JOINs", ["FULL OUTER JOIN", "mismatches"], ["customers", "orders", "shipments"], [
        ...
        Task("l9f-t2", "Show all customers and all orders using a FULL JOIN. Include unmatched rows from both sides. Show customer name and order id, sorted by order id.", "", "SELECT customers.name, orders.id FROM customers FULL JOIN orders ON orders.customer_id = customers.id ORDER BY orders.id;"),
    ...
    Lesson("self-joins", 11, "Self Joins", ["self join", "hierarchies"], ["employees", "customers"], [
            ...
            Task("l30-t3", "Find pairs of customers from the same country without duplicate mirrored pairs. Show customer_a, customer_b, and country. Sort by country.", "", "SELECT a.name AS customer_a, b.name AS customer_b, a.country FROM customers a JOIN customers b ON a.country = b.country AND a.id < b.id ORDER BY a.country;"),
            Task("l30-t4", "Find employees who share a manager (exclude NULL managers and duplicate mirrored pairs). Show employee_a, employee_b, and manager_id. Sort by employee_a.", "", "SELECT a.name AS employee_a, b.name AS employee_b, a.manager_id FROM employees a JOIN employees b ON a.manager_id = b.manager_id AND a.id < b.id WHERE a.manager_id IS NOT NULL ORDER BY employee_a;"),
            Task("l30-t5", "Find customers who placed two distinct orders through the same channel without duplicate mirrored pairs. Show customer_name, channel, order_a_id, and order_b_id. Sort by customer_name then order_a_id.", "", "SELECT c.name AS customer_name, a.channel, a.id AS order_a_id, b.id AS order_b_id FROM orders a JOIN orders b ON a.customer_id = b.customer_id AND a.channel = b.channel AND a.id < b.id JOIN customers c ON c.id = a.customer_id ORDER BY customer_name, order_a_id;"),
        ]),
    Lesson("semi-anti-joins", 12, "Semi-Joins, Anti-Joins & EXISTS", ["EXISTS", "NOT EXISTS", "LEFT SEMI", "RIGHT SEMI"], ["customers", "orders", "products", "order_items", "shipments", "employees", "support_tickets"], [
            ...
            Task("l29-t2", "Find customers who have never had a cancelled order using NOT EXISTS. Show name, sorted by name.", "", "SELECT name FROM customers WHERE NOT EXISTS (SELECT 1 FROM orders WHERE orders.customer_id = customers.id AND orders.status = 'cancelled') ORDER BY name;"),
            ...
            Task("l29-t6", "Left Semi Join: Return only the customer names who have placed at least one order. Use EXISTS (not JOIN). Sort by name.", "", "SELECT name FROM customers WHERE EXISTS (SELECT 1 FROM orders WHERE orders.customer_id = customers.id) ORDER BY name;"),
            Task("l29-t7", "Left Semi Join using IN: Find product names that appear in at least one order item. Use an IN subquery instead of EXISTS. Sort by name.", "", "SELECT name FROM products WHERE id IN (SELECT product_id FROM order_items) ORDER BY name;"),
            Task("l29-t8", "Right Semi Join: Return the order IDs where the associated customer is from 'USA'. Use EXISTS to filter orders based on matching customers. Sort by id.", "", "SELECT id FROM orders WHERE EXISTS (SELECT 1 FROM customers WHERE customers.id = orders.customer_id AND customers.country = 'USA') ORDER BY id;"),
            Task("l29-t9", "Right Semi Join using IN: Find customer IDs that have submitted at least one support ticket. Use IN. Sort by id.", "", "SELECT id FROM customers WHERE id IN (SELECT customer_id FROM support_tickets) ORDER BY id;"),
            Task("l29-t10", "Left Semi Join: Find categories (distinct) from the products table that have at least one product ordered by a customer from 'India'. Use EXISTS with correlated subqueries. Sort by category.", "", "SELECT DISTINCT category FROM products WHERE EXISTS (SELECT 1 FROM order_items JOIN orders ON orders.id = order_items.order_id JOIN customers ON customers.id = orders.customer_id WHERE order_items.product_id = products.id AND customers.country = 'India') ORDER BY category;"),
            Task("l29-t11", "Left Anti Join: Find customers who have never submitted a support ticket. Use NOT EXISTS. Sort by name.", "", "SELECT name FROM customers WHERE NOT EXISTS (SELECT 1 FROM support_tickets WHERE support_tickets.customer_id = customers.id) ORDER BY name;"),
            Task("l29-t12", "Right Semi Join: Return shipment IDs where the shipped order contains a product from the 'Electronics' category. Use EXISTS. Sort by id.", "", "SELECT id FROM shipments WHERE EXISTS (SELECT 1 FROM order_items JOIN products ON products.id = order_items.product_id WHERE order_items.order_id = shipments.order_id AND products.category = 'Electronics') ORDER BY id;"),
            Task("l29-t13", "Left Semi Join vs DISTINCT JOIN: Find customer names who have ordered 'Mechanical Keyboard'. Use IN with a chained subquery (no JOINs in the outer query). Sort by name.", "", "SELECT name FROM customers WHERE id IN (SELECT customer_id FROM orders WHERE id IN (SELECT order_id FROM order_items WHERE product_id = (SELECT id FROM products WHERE name = 'Mechanical Keyboard'))) ORDER BY name;"),
            Task("l29-t14", "Right Anti Join: Find order IDs that do NOT have any shipment. Use NOT IN. Sort by id.", "", "SELECT id FROM orders WHERE id NOT IN (SELECT order_id FROM shipments) ORDER BY id;"),
            Task("l29-t15", "Left Semi Join with aggregation: Find customer names who have placed 2 or more orders. Use EXISTS with a correlated subquery that uses GROUP BY and HAVING. Sort by name.", "", "SELECT name FROM customers WHERE EXISTS (SELECT 1 FROM orders WHERE orders.customer_id = customers.id GROUP BY orders.customer_id HAVING COUNT(*) >= 2) ORDER BY name;"),
        ]),
    Lesson("join-patterns", 13, "Join Interview Scenarios", ["identify the join"], ["customers", "orders", "products"], [
        ...
        Task("l10j-t3", "Scenario: 'Find instances where two different customers live in the exact same city. Show name_a, name_b, and city without duplicate mirrored pairs.' Sort by city then name_a.", "", "SELECT a.name AS name_a, b.name AS name_b, a.city FROM customers a JOIN customers b ON a.city = b.city AND a.id < b.id ORDER BY a.city, name_a;"),
        ...
        Task("l10j-t5", "Scenario: 'Build a complete picture: Show every customer alongside any orders they may have, even if a customer never ordered.' Return customer name and order id, sorted by customer name.", "", "SELECT customers.name, orders.id FROM customers LEFT JOIN orders ON orders.customer_id = customers.id ORDER BY customers.name;"),
    ]),
    Lesson("joins-capstone", 14, "Joins: Interview Capstone", ["Hint-Free", "Data Modeling"], ["customers", "orders", "order_items", "products", "shipments", "employees", "support_tickets"], [
        Task("jc-t1", "Identify customers who have placed an order but none of their orders have been delivered yet. Return their name.", "", "SELECT DISTINCT customers.name FROM customers JOIN orders ON orders.customer_id = customers.id WHERE customers.id NOT IN (SELECT customer_id FROM orders WHERE status = 'delivered');"),
        ...
        Task("jc-t11", "List any support tickets that are unassigned to an order or are still open. Return ticket id and priority.", "", "SELECT id, priority FROM support_tickets WHERE order_id IS NULL OR status = 'open';"),
        ...
        Task("jc-t14", "Find customers whose orders were shipped via 'ShipFast'. Return unique customer names.", "", "SELECT DISTINCT customers.name FROM customers JOIN orders ON orders.customer_id = customers.id JOIN shipments ON shipments.order_id = orders.id WHERE shipments.carrier = 'ShipFast';"),
        ...
        Task("jc-t16", "Determine which customers have submitted support tickets. Return the customer name.", "", "SELECT DISTINCT customers.name FROM customers JOIN support_tickets ON support_tickets.customer_id = customers.id;"),
        Task("jc-t17", "List all order IDs that contain the product 'USB-C Hub'.", "", "SELECT DISTINCT order_items.order_id FROM order_items JOIN products ON products.id = order_items.product_id WHERE products.name = 'USB-C Hub';"),
    ...
    Lesson("expressions-capstone", 22, "Expressions & Types: Interview Capstone", ["Hint-Free", "Data Cleaning"], ["customers", "orders", "products"], [
        Task("ec-t1", "Extract the country code (first 2 characters) from every customer's country. Return name and 'code'.", "", "SELECT name, SUBSTR(country, 1, 2) AS code FROM customers;"),
        ...
        Task("ec-t6", "Replace the word 'Desk' with 'Desktop' in any product name that contains it. Return original name and 'new_name'.", "", "SELECT name, REPLACE(name, 'Desk', 'Desktop') AS new_name FROM products WHERE name LIKE '%Desk%';"),
    ...
    Lesson("grouping", 24, "GROUP BY and HAVING", ["GROUP BY", "HAVING"], ["products", "orders", "order_items", "customers"], [
        ...
        Task("l11-t6", "Find order channels that have at least 2 orders placed through them. Show channel and count as 'order_count', sorted by channel.", "", "SELECT channel, COUNT(*) AS order_count FROM orders GROUP BY channel HAVING COUNT(*) >= 2 ORDER BY channel;"),
        Task("l11-t7", "Count the total number of orders placed per customer country. Join customers with orders. Show country and count as 'order_count', sorted by order_count descending then country.", "", "SELECT customers.country, COUNT(DISTINCT orders.id) AS order_count FROM customers JOIN orders ON orders.customer_id = customers.id GROUP BY customers.country ORDER BY order_count DESC, customers.country;"),
        Task("l11-t8", "Find the total revenue generated by each product category. Join products with order_items. Show category and total as 'category_revenue' (rounded to 2 decimals), sorted by category_revenue descending.", "", "SELECT products.category, ROUND(SUM(order_items.quantity * order_items.unit_price), 2) AS category_revenue FROM products JOIN order_items ON order_items.product_id = products.id GROUP BY products.category ORDER BY category_revenue DESC;"),
        Task("l11-t9", "Find order statuses where at least 2 distinct products were ordered. Join orders with order_items. Show status and count as 'unique_products', sorted by status.", "", "SELECT orders.status, COUNT(DISTINCT order_items.product_id) AS unique_products FROM orders JOIN order_items ON order_items.order_id = orders.id GROUP BY orders.status HAVING COUNT(DISTINCT order_items.product_id) >= 2 ORDER BY orders.status;"),
        Task("l11-t10", "Find customers who have ordered from more than 1 distinct product category. Join customers, orders, order_items, and products. Show customer name and count as 'categories_ordered', sorted by categories_ordered descending then name.", "", "SELECT customers.name, COUNT(DISTINCT products.category) AS categories_ordered FROM customers JOIN orders ON orders.customer_id = customers.id JOIN order_items ON order_items.order_id = orders.id JOIN products ON products.id = order_items.product_id GROUP BY customers.id, customers.name HAVING COUNT(DISTINCT products.category) > 1 ORDER BY categories_ordered DESC, customers.name;"),
        Task("l11-t11", "Find shipping carriers whose average shipping cost exceeds 6.00. Show carrier and average as 'avg_cost' (rounded to 2 decimals), sorted by avg_cost descending.", "", "SELECT carrier, ROUND(AVG(shipping_cost), 2) AS avg_cost FROM shipments GROUP BY carrier HAVING AVG(shipping_cost) > 6 ORDER BY avg_cost DESC;"),
        Task("l11-t12", "For each product category, count how many distinct customer countries have purchased from it. Join products, order_items, orders, and customers. Show category and count as 'country_count'. Only include categories bought by 2 or more countries. Sort by country_count descending then category.", "", "SELECT products.category, COUNT(DISTINCT customers.country) AS country_count FROM products JOIN order_items ON order_items.product_id = products.id JOIN orders ON orders.id = order_items.order_id JOIN customers ON customers.id = orders.customer_id GROUP BY products.category HAVING COUNT(DISTINCT customers.country) >= 2 ORDER BY country_count DESC, products.category;"),
        Task("l11-t13", "Find customers whose total lifetime spend exceeds 100. Join customers, orders, and order_items. Show name, total as 'total_spend' (rounded to 2 decimals), and count of distinct orders as 'order_count'. Sort by total_spend descending.", "", "SELECT customers.name, ROUND(SUM(order_items.quantity * order_items.unit_price), 2) AS total_spend, COUNT(DISTINCT orders.id) AS order_count FROM customers JOIN orders ON orders.customer_id = customers.id JOIN order_items ON order_items.order_id = orders.id GROUP BY customers.id, customers.name HAVING SUM(order_items.quantity * order_items.unit_price) > 100 ORDER BY total_spend DESC;"),
        Task("l11-t14", "Find months where the total revenue exceeded 100. Use substr(ordered_at, 1, 7) as 'order_month'. Join orders and order_items. Show order_month and total as 'monthly_revenue' (rounded to 2 decimals), sorted by order_month.", "", "SELECT substr(orders.ordered_at, 1, 7) AS order_month, ROUND(SUM(order_items.quantity * order_items.unit_price), 2) AS monthly_revenue FROM orders JOIN order_items ON order_items.order_id = orders.id GROUP BY substr(orders.ordered_at, 1, 7) HAVING SUM(order_items.quantity * order_items.unit_price) > 100 ORDER BY order_month;"),
        Task("l11-t15", "Calculate the average order value per customer tier. Only include tiers with at least 2 distinct orders. Join customers, orders, and order_items. Show tier and average as 'avg_order_value' (rounded to 2 decimals). Compute as total revenue divided by number of distinct orders. Sort by avg_order_value descending.", "", "SELECT customers.tier, ROUND(SUM(order_items.quantity * order_items.unit_price) / COUNT(DISTINCT orders.id), 2) AS avg_order_value FROM customers JOIN orders ON orders.customer_id = customers.id JOIN order_items ON order_items.order_id = orders.id GROUP BY customers.tier HAVING COUNT(DISTINCT orders.id) >= 2 ORDER BY avg_order_value DESC;"),
        Task("l11-t16", "For each product category, show the min unit price sold (as 'min_price'), max unit price sold (as 'max_price'), and the difference as 'price_spread' (rounded to 2 decimals). Only include categories where at least 2 distinct products were ordered. Join products and order_items. Sort by price_spread descending.", "", "SELECT products.category, MIN(order_items.unit_price) AS min_price, MAX(order_items.unit_price) AS max_price, ROUND(MAX(order_items.unit_price) - MIN(order_items.unit_price), 2) AS price_spread FROM products JOIN order_items ON order_items.product_id = products.id GROUP BY products.category HAVING COUNT(DISTINCT products.id) >= 2 ORDER BY price_spread DESC;"),
        Task("l11-t17", "Find customers who have used exactly 1 order channel across all their orders. Join customers with orders. Show name and that single channel as 'channels_used'. Sort by name.", "", "SELECT customers.name, COUNT(DISTINCT orders.channel) AS channels_used FROM customers JOIN orders ON orders.customer_id = customers.id GROUP BY customers.id, customers.name HAVING COUNT(DISTINCT orders.channel) = 1 ORDER BY customers.name;"),
    ]),
    Lesson("conditional-aggregation", 25, "Conditional Aggregation", ["SUM(CASE)", "COUNT(CASE)"], ["orders", "order_items", "customers", "products"], [
            ...
            Task("l23-t6", "For each product category, count items sold at full price (unit_price = product price, as 'full_price_items') and items sold below full price (as 'discounted_items'). Join products and order_items. Sort by category.", "", "SELECT products.category, SUM(CASE WHEN order_items.unit_price < products.price THEN 1 ELSE 0 END) AS discounted_items, SUM(CASE WHEN order_items.unit_price = products.price THEN 1 ELSE 0 END) AS full_price_items FROM products JOIN order_items ON order_items.product_id = products.id GROUP BY products.category ORDER BY products.category;"),
            Task("l23-t7", "For each customer, show how many of their orders are delivered (as 'delivered'), cancelled (as 'cancelled'), and in progress meaning processing or shipped (as 'in_progress'). Join customers with orders. Sort by customer name.", "", "SELECT customers.name, SUM(CASE WHEN orders.status = 'delivered' THEN 1 ELSE 0 END) AS delivered, SUM(CASE WHEN orders.status = 'cancelled' THEN 1 ELSE 0 END) AS cancelled, SUM(CASE WHEN orders.status NOT IN ('delivered', 'cancelled') THEN 1 ELSE 0 END) AS in_progress FROM customers JOIN orders ON orders.customer_id = customers.id GROUP BY customers.id, customers.name ORDER BY customers.name;"),
            Task("l23-t8", "Break down monthly revenue into delivered vs pending buckets. Show month (substr of ordered_at, 7 chars, as 'order_month'), delivered revenue as 'delivered_revenue' and non-delivered revenue as 'pending_revenue', both rounded to 2 decimals. Join orders and order_items. Sort by order_month.", "", "SELECT substr(orders.ordered_at, 1, 7) AS order_month, ROUND(SUM(CASE WHEN orders.status = 'delivered' THEN order_items.quantity * order_items.unit_price ELSE 0 END), 2) AS delivered_revenue, ROUND(SUM(CASE WHEN orders.status <> 'delivered' THEN order_items.quantity * order_items.unit_price ELSE 0 END), 2) AS pending_revenue FROM orders JOIN order_items ON order_items.order_id = orders.id GROUP BY substr(orders.ordered_at, 1, 7) ORDER BY order_month;"),
            Task("l23-t9", "For each product category, compute units sold where the product rating is 4.5 or above (as 'premium_units') vs below 4.5 (as 'standard_units'). Join products and order_items. Sort by category.", "", "SELECT products.category, SUM(CASE WHEN products.rating >= 4.5 THEN order_items.quantity ELSE 0 END) AS premium_units, SUM(CASE WHEN products.rating < 4.5 THEN order_items.quantity ELSE 0 END) AS standard_units FROM products JOIN order_items ON order_items.product_id = products.id GROUP BY products.category ORDER BY products.category;"),
            Task("l23-t10", "For each customer tier, break down total revenue by order channel: 'web_revenue', 'mobile_revenue', and 'store_revenue', all rounded to 2 decimals. Join customers, orders, and order_items. Sort by tier.", "", "SELECT customers.tier, ROUND(SUM(CASE WHEN orders.channel = 'web' THEN order_items.quantity * order_items.unit_price ELSE 0 END), 2) AS web_revenue, ROUND(SUM(CASE WHEN orders.channel = 'mobile' THEN order_items.quantity * order_items.unit_price ELSE 0 END), 2) AS mobile_revenue, ROUND(SUM(CASE WHEN orders.channel = 'store' THEN order_items.quantity * order_items.unit_price ELSE 0 END), 2) AS store_revenue FROM customers JOIN orders ON orders.customer_id = customers.id JOIN order_items ON order_items.order_id = orders.id GROUP BY customers.tier ORDER BY customers.tier;"),
            Task("l23-t11", "Calculate the overall delivery rate and cancellation rate as percentages of total orders. Show 'delivery_rate' and 'cancellation_rate', both rounded to 2 decimals. No GROUP BY needed — single-row result.", "", "SELECT ROUND(SUM(CASE WHEN status = 'delivered' THEN 1 ELSE 0 END) * 100.0 / COUNT(*), 2) AS delivery_rate, ROUND(SUM(CASE WHEN status = 'cancelled' THEN 1 ELSE 0 END) * 100.0 / COUNT(*), 2) AS cancellation_rate FROM orders;"),
            Task("l23-t12", "For each customer, count items where the product price exceeds 50 (as 'expensive_items') and items where it does not (as 'affordable_items'). Join customers, orders, order_items, and products. Sort by customer name.", "", "SELECT customers.name, SUM(CASE WHEN products.price > 50 THEN order_items.quantity ELSE 0 END) AS expensive_items, SUM(CASE WHEN products.price <= 50 THEN order_items.quantity ELSE 0 END) AS affordable_items FROM customers JOIN orders ON orders.customer_id = customers.id JOIN order_items ON order_items.order_id = orders.id JOIN products ON products.id = order_items.product_id GROUP BY customers.id, customers.name ORDER BY customers.name;"),
            Task("l23-t13", "For each product category, classify products into stock buckets: 'out_of_stock' (stock = 0), 'low_stock' (stock 1 to 20), and 'in_stock' (stock > 20). Use COUNT(CASE ...). Sort by category.", "", "SELECT category, COUNT(CASE WHEN stock = 0 THEN 1 END) AS out_of_stock, COUNT(CASE WHEN stock BETWEEN 1 AND 20 THEN 1 END) AS low_stock, COUNT(CASE WHEN stock > 20 THEN 1 END) AS in_stock FROM products GROUP BY category ORDER BY category;"),
            Task("l23-t14", "For each customer country, calculate delivered revenue (as 'delivered_revenue'), total revenue (as 'total_revenue'), and the percentage of revenue that is delivered (as 'delivery_pct'), all rounded to 2 decimals. Join customers, orders, and order_items. Sort by delivery_pct descending.", "", "SELECT customers.country, ROUND(SUM(CASE WHEN orders.status = 'delivered' THEN order_items.quantity * order_items.unit_price ELSE 0 END), 2) AS delivered_revenue, ROUND(SUM(order_items.quantity * order_items.unit_price), 2) AS total_revenue, ROUND(SUM(CASE WHEN orders.status = 'delivered' THEN order_items.quantity * order_items.unit_price ELSE 0 END) * 100.0 / SUM(order_items.quantity * order_items.unit_price), 2) AS delivery_pct FROM customers JOIN orders ON orders.customer_id = customers.id JOIN order_items ON order_items.order_id = orders.id GROUP BY customers.country ORDER BY delivery_pct DESC;"),
            Task("l23-t15", "For each support ticket priority level, show the count of closed tickets (as 'closed_count'), count of active tickets meaning open or pending (as 'active_count'), and the resolution rate as a percentage (as 'resolution_rate', rounded to 2 decimals). Sort by priority.", "", "SELECT priority, SUM(CASE WHEN status = 'closed' THEN 1 ELSE 0 END) AS closed_count, SUM(CASE WHEN status IN ('open', 'pending') THEN 1 ELSE 0 END) AS active_count, ROUND(SUM(CASE WHEN status = 'closed' THEN 1 ELSE 0 END) * 100.0 / COUNT(*), 2) AS resolution_rate FROM support_tickets GROUP BY priority ORDER BY priority;"),
            Task("l23-t16", "Find customers who have BOTH delivered revenue > 0 AND non-delivered revenue > 0. Show name, 'delivered_revenue' and 'non_delivered_revenue' (both rounded to 2 decimals). Use HAVING with conditional aggregates. Join customers, orders, and order_items. Sort by name.", "", "SELECT customers.name, ROUND(SUM(CASE WHEN orders.status = 'delivered' THEN order_items.quantity * order_items.unit_price ELSE 0 END), 2) AS delivered_revenue, ROUND(SUM(CASE WHEN orders.status <> 'delivered' THEN order_items.quantity * order_items.unit_price ELSE 0 END), 2) AS non_delivered_revenue FROM customers JOIN orders ON orders.customer_id = customers.id JOIN order_items ON order_items.order_id = orders.id GROUP BY customers.id, customers.name HAVING SUM(CASE WHEN orders.status = 'delivered' THEN order_items.quantity * order_items.unit_price ELSE 0 END) > 0 AND SUM(CASE WHEN orders.status <> 'delivered' THEN order_items.quantity * order_items.unit_price ELSE 0 END) > 0 ORDER BY customers.name;"),
            Task("l23-t17", "Build a revenue pipeline by product category: show 'delivered_revenue', 'lost_revenue' (cancelled), and 'pipeline_revenue' (processing + shipped), all rounded to 2 decimals. Join products, order_items, and orders. Sort by category.", "", "SELECT products.category, ROUND(SUM(CASE WHEN orders.status = 'delivered' THEN order_items.quantity * order_items.unit_price ELSE 0 END), 2) AS delivered_revenue, ROUND(SUM(CASE WHEN orders.status = 'cancelled' THEN order_items.quantity * order_items.unit_price ELSE 0 END), 2) AS lost_revenue, ROUND(SUM(CASE WHEN orders.status IN ('processing', 'shipped') THEN order_items.quantity * order_items.unit_price ELSE 0 END), 2) AS pipeline_revenue FROM products JOIN order_items ON order_items.product_id = products.id JOIN orders ON orders.id = order_items.order_id GROUP BY products.category ORDER BY products.category;"),
        ]),
    Lesson("aggregations-capstone", 26, "Aggregations: Interview Capstone", ["Hint-Free", "Data Analytics"], ["customers", "orders", "order_items", "products", "shipments", "employees", "support_tickets"], [
        ...
        Task("ac-t3", "Count the number of support tickets per priority level. Return priority and ticket_count, sorted by ticket_count descending.", "", "SELECT priority, COUNT(*) AS ticket_count FROM support_tickets GROUP BY priority ORDER BY ticket_count DESC;"),
        ...
        Task("ac-t11", "Determine the average time to ship (shipped_at - ordered_at in days) for ShipFast shipments. Return avg_ship_days rounded to 1 decimal.", "", "SELECT ROUND(AVG(julianday(shipments.shipped_at) - julianday(orders.ordered_at)), 1) AS avg_ship_days FROM shipments JOIN orders ON orders.id = shipments.order_id WHERE shipments.carrier = 'ShipFast';"),
        ...
        Task("ac-t17", "Identify product categories where the average price exceeds 50. Return category and avg_price.", "", "SELECT category, AVG(price) AS avg_price FROM products GROUP BY category HAVING AVG(price) > 50;"),
        Task("ac-t18", "For each customer, calculate the total number of support tickets they have opened. Return customer_id and ticket_count, sorted by ticket_count descending.", "", "SELECT customer_id, COUNT(*) AS ticket_count FROM support_tickets GROUP BY customer_id ORDER BY ticket_count DESC;"),
    ...
    Lesson("subqueries", 30, "Subqueries", ["subquery", "nested queries", "EXISTS", "correlated"], ["products", "customers", "orders", "order_items", "shipments", "employees", "support_tickets"], [
        Task("sub-t1", "Find products whose price is strictly greater than the average price of products in their own category. Return product name, category, and price, sorted by category, then price descending, then name.", "", "SELECT name, category, price FROM products p1 WHERE price > (SELECT AVG(price) FROM products p2 WHERE p2.category = p1.category) ORDER BY category, price DESC, name;"),
        Task("sub-t2", "Identify customers who have never placed a 'cancelled' order. Return customer name, sorted by name.", "", "SELECT name FROM customers WHERE id NOT IN (SELECT DISTINCT customer_id FROM orders WHERE status = 'cancelled') ORDER BY name;"),
        Task("sub-t3", "Find the name of the customer who has spent the most money overall on a single order (based on quantity multiplied by unit price). If there is a tie, return the customer name that comes first alphabetically.", "", "SELECT name FROM customers WHERE id = (SELECT customer_id FROM orders JOIN order_items ON order_items.order_id = orders.id GROUP BY orders.id ORDER BY SUM(quantity * unit_price) DESC, customer_id LIMIT 1);"),
        Task("sub-t4", "For the 'Electronics' category, find the second highest product price (as 'second_highest_price').", "", "SELECT MAX(price) AS second_highest_price FROM products WHERE category = 'Electronics' AND price < (SELECT MAX(price) FROM products WHERE category = 'Electronics');"),
        Task("sub-t5", "Find products whose price is strictly greater than the average price of all products ordered by customers from 'Singapore'. Return product name and price, sorted by price descending, then name.", "", "SELECT name, price FROM products WHERE price > (SELECT AVG(price) FROM products WHERE id IN (SELECT DISTINCT product_id FROM order_items JOIN orders ON orders.id = order_items.order_id JOIN customers ON customers.id = orders.customer_id WHERE customers.country = 'Singapore')) ORDER BY price DESC, name;"),
        Task("sub-t6", "List customers whose total lifetime spend (quantity × unit_price) is strictly greater than the average lifetime spend of all customers who have placed at least one order. Return customer name, sorted by name.", "", "SELECT name FROM customers WHERE id IN (SELECT customer_id FROM orders JOIN order_items ON order_items.order_id = orders.id GROUP BY customer_id HAVING SUM(quantity * unit_price) > (SELECT AVG(lifetime_spend) FROM (SELECT SUM(quantity * unit_price) AS lifetime_spend FROM orders JOIN order_items ON order_items.order_id = orders.id GROUP BY customer_id))) ORDER BY name;"),
        Task("sub-t7", "Find products whose current stock is strictly greater than the average stock of all products purchased by customer 'Asha Rao'. Return product name and stock, sorted by stock descending.", "", "SELECT name, stock FROM products WHERE stock > (SELECT AVG(stock) FROM products WHERE id IN (SELECT DISTINCT product_id FROM order_items JOIN orders ON orders.id = order_items.order_id JOIN customers ON customers.id = orders.customer_id WHERE customers.name = 'Asha Rao')) ORDER BY stock DESC;"),
        Task("sub-t8", "Find employees who earn strictly more than the average salary of their own team, but strictly less than their direct manager's salary. Return employee name and salary, sorted by salary descending.", "", "SELECT name, salary FROM employees e1 WHERE salary > (SELECT AVG(salary) FROM employees e2 WHERE e2.team = e1.team) AND salary < (SELECT salary FROM employees WHERE id = e1.manager_id) ORDER BY salary DESC;"),
        Task("sub-t9", "Find support tickets that were opened on or after the date the corresponding order was placed. Return ticket id, sorted by ticket id.", "", "SELECT id FROM support_tickets st WHERE opened_at >= (SELECT ordered_at FROM orders WHERE id = st.order_id) ORDER BY id;"),
        Task("sub-t10", "Identify product categories where the highest priced product is at least 4 times the price of the lowest priced product in that same category. Return category, sorted alphabetically.", "", "SELECT category FROM products GROUP BY category HAVING MAX(price) >= 4 * MIN(price) ORDER BY category;"),
        Task("sub-t11", "Find the name of the customer who has the highest number of 'high' priority support tickets. If there is a tie, return the customer name that comes first alphabetically.", "", "SELECT name FROM customers WHERE id = (SELECT customer_id FROM support_tickets WHERE priority = 'high' GROUP BY customer_id ORDER BY COUNT(*) DESC, customer_id LIMIT 1);"),
        Task("sub-t12", "Find orders where every single item purchased in that order has a unit price strictly less than the overall average price of all products in the products table. Return order id, sorted by order id.", "", "SELECT id FROM orders WHERE id NOT IN (SELECT DISTINCT order_id FROM order_items WHERE unit_price >= (SELECT AVG(price) FROM products)) AND id IN (SELECT DISTINCT order_id FROM order_items) ORDER BY id;"),
        Task("sub-t13", "Find the employee with the highest salary among those who do not manage anyone. Return employee name and salary, sorted by salary descending, then name.", "", "SELECT name, salary FROM employees WHERE id NOT IN (SELECT DISTINCT manager_id FROM employees WHERE manager_id IS NOT NULL) ORDER BY salary DESC, name LIMIT 1;"),
        Task("sub-t14", "For each customer who has placed at least one order, find their name and the name of their single most expensive purchased product (based on unit price in order_items). If there is a tie in price for a customer, resolve it by selecting the product name that comes first alphabetically. Return customer name and product name (as 'max_product'), sorted by customer name.", "", "SELECT c.name, (SELECT p.name FROM products p JOIN order_items oi ON oi.product_id = p.id JOIN orders o ON o.id = oi.order_id WHERE o.customer_id = c.id ORDER BY oi.unit_price DESC, p.name LIMIT 1) AS max_product FROM customers c WHERE EXISTS (SELECT 1 FROM orders WHERE customer_id = c.id) ORDER BY c.name;"),
        Task("sub-t15", "Find order IDs that contain the product with the absolute lowest rating in the 'Electronics' category. Return order_id, sorted by order_id.", "", "SELECT DISTINCT order_id FROM order_items WHERE product_id IN (SELECT id FROM products WHERE category = 'Electronics' AND rating = (SELECT MIN(rating) FROM products WHERE category = 'Electronics')) ORDER BY order_id;"),
        Task("sub-t16", "Find customers who have referred at least one other customer who went on to place an order that was successfully delivered. Return customer name, sorted by name.", "", "SELECT name FROM customers c1 WHERE EXISTS (SELECT 1 FROM customers c2 JOIN orders o ON o.customer_id = c2.id JOIN shipments s ON s.order_id = o.id WHERE c2.referrer_id = c1.id AND s.delivered_at IS NOT NULL) ORDER BY name;"),
        Task("sub-t17", "Calculate the percentage of overall order items revenue (sum of quantity * unit price) generated by orders that have been delivered, relative to all order items. Return the percentage as 'delivered_pct', rounded to 2 decimal places.", "", "SELECT ROUND((SELECT SUM(quantity * unit_price) FROM order_items JOIN orders ON orders.id = order_items.order_id WHERE orders.status = 'delivered') * 100.0 / (SELECT SUM(quantity * unit_price) FROM order_items), 2) AS delivered_pct;"),
        Task("sub-t18", "Find shipments whose shipping cost is strictly higher than the average shipping cost of all shipments for orders placed in the same calendar month. Return shipment order_id, sorted by order_id.", "", "SELECT s1.order_id FROM shipments s1 JOIN orders o1 ON o1.id = s1.order_id WHERE s1.shipping_cost > (SELECT AVG(s2.shipping_cost) FROM shipments s2 JOIN orders o2 ON o2.id = s2.order_id WHERE substr(o2.ordered_at, 1, 7) = substr(o1.ordered_at, 1, 7)) ORDER BY s1.order_id;"),
        Task("sub-t19", "List countries where the average total spending per customer (total revenue from that country divided by the number of unique customers from that country who placed orders) is strictly above the overall average customer spending across the entire platform. Return country, sorted alphabetically.", "", "SELECT country FROM customers JOIN orders ON orders.customer_id = customers.id JOIN order_items ON order_items.order_id = orders.id GROUP BY country HAVING SUM(quantity * unit_price) / COUNT(DISTINCT customers.id) > (SELECT SUM(quantity * unit_price) / COUNT(DISTINCT customer_id) FROM orders JOIN order_items ON order_items.order_id = orders.id) ORDER BY country;"),
        Task("sub-t20", "Identify support tickets that took strictly longer to resolve (resolved_at minus opened_at in days) than the average resolution time of all resolved tickets of the same priority level. Return ticket id, sorted by ticket id.", "", "SELECT id FROM support_tickets st1 WHERE resolved_at IS NOT NULL AND (julianday(resolved_at) - julianday(opened_at)) > (SELECT AVG(julianday(st2.resolved_at) - julianday(st2.opened_at)) FROM support_tickets st2 WHERE st2.resolved_at IS NOT NULL) ORDER BY id;"),
        Task("sub-t21", "Find products that have never been ordered via the 'store' channel. Return product name, sorted by name.", "", "SELECT name FROM products WHERE id NOT IN (SELECT DISTINCT product_id FROM order_items JOIN orders ON orders.id = order_items.order_id WHERE orders.channel = 'store') ORDER BY name;"),
        Task("sub-t22", "Identify customers who were referred by someone who was also referred by a customer. Return customer name, sorted by name.", "", "SELECT name FROM customers WHERE referrer_id IN (SELECT id FROM customers WHERE referrer_id IS NOT NULL) ORDER BY name;"),
        Task("sub-t23", "Find products whose current stock is strictly less than 15% of the total quantity of that product ever sold. Return product name, sorted by name.", "", "SELECT name FROM products p WHERE stock < 0.15 * (SELECT COALESCE(SUM(quantity), 0) FROM order_items WHERE product_id = p.id) ORDER BY name;"),
        Task("sub-t24", "Find the employee name of the manager whose direct reports have the highest average salary. If there is a tie, return the manager name that comes first alphabetically.", "", "SELECT name FROM employees WHERE id = (SELECT manager_id FROM employees WHERE manager_id IS NOT NULL GROUP BY manager_id ORDER BY AVG(salary) DESC, manager_id LIMIT 1);"),
        Task("sub-t25", "Find customer names who placed at least one order in January 2024 but did not place any orders in March 2024. Return customer name, sorted by name.", "", "SELECT DISTINCT name FROM customers JOIN orders ON orders.customer_id = customers.id WHERE ordered_at LIKE '2024-01-%' EXCEPT SELECT DISTINCT name FROM customers JOIN orders ON orders.customer_id = customers.id WHERE ordered_at LIKE '2024-03-%' ORDER BY name;"),
        Task("sub-t26", "Find customers who have placed orders with a higher cumulative quantity of items than the customer 'Mia Chen' has purchased. Return customer name, sorted by name.", "", "SELECT name FROM customers JOIN orders ON orders.customer_id = customers.id JOIN order_items ON order_items.order_id = orders.id GROUP BY customers.id, customers.name HAVING SUM(quantity) > (SELECT SUM(quantity) FROM orders JOIN order_items ON order_items.order_id = orders.id JOIN customers ON customers.id = orders.customer_id WHERE customers.name = 'Mia Chen') ORDER BY name;"),
        Task("sub-t27", "Find the product category that has the largest absolute difference between its average product price and the overall average product price of all products. Return category.", "", "SELECT category FROM products GROUP BY category ORDER BY ABS(AVG(price) - (SELECT AVG(price) FROM products)) DESC LIMIT 1;"),
        Task("sub-t28", "Find the order ID and customer name for orders that contain strictly more items in total (sum of quantities) than the average number of items per order across all orders. Return order id and customer name, sorted by order id.", "", "SELECT orders.id, customers.name FROM orders JOIN customers ON customers.id = orders.customer_id JOIN order_items ON order_items.order_id = orders.id GROUP BY orders.id, customers.name HAVING SUM(quantity) > (SELECT AVG(order_qty) FROM (SELECT SUM(quantity) AS order_qty FROM order_items GROUP BY order_id)) ORDER BY orders.id;"),
        Task("sub-t29", "Find customer names who have ordered products from every category that 'Noah Kim' has ordered from. Exclude Noah Kim himself. Return customer name, sorted by name.", "", "SELECT name FROM customers WHERE name <> 'Noah Kim' AND id IN (SELECT customer_id FROM orders JOIN order_items ON order_items.order_id = orders.id JOIN products ON products.id = order_items.product_id WHERE products.category IN (SELECT DISTINCT category FROM products JOIN order_items ON order_items.product_id = products.id JOIN orders ON orders.id = order_items.order_id JOIN customers ON customers.id = orders.customer_id WHERE customers.name = 'Noah Kim') GROUP BY customer_id HAVING COUNT(DISTINCT products.category) = (SELECT COUNT(DISTINCT category) FROM products JOIN order_items ON order_items.product_id = products.id JOIN orders ON orders.id = order_items.order_id JOIN customers ON customers.id = orders.customer_id WHERE customers.name = 'Noah Kim')) ORDER BY name;"),
        Task("sub-t30", "Find the customer name who has the longest gap in days between any two of their consecutive orders. Return name, sorted by name.", "", "SELECT name FROM customers JOIN (SELECT customer_id, gap, MAX(gap) OVER() AS max_gap FROM (SELECT customer_id, julianday(ordered_at) - julianday(LAG(ordered_at) OVER(PARTITION BY customer_id ORDER BY ordered_at)) AS gap FROM orders)) AS Gaps ON Gaps.customer_id = customers.id WHERE Gaps.gap = Gaps.max_gap ORDER BY name;"),
    ]),
    Lesson("sets-capstone", 31, "Sets & Subqueries: Interview Capstone", ["Hint-Free", "Logic"], ["customers", "orders", "products"], [
        ...
        Task("sc-t9", "Find customers who reside in the same country as the customer named 'Asha Rao'. Exclude Asha Rao herself.", "", "SELECT name FROM customers WHERE country = (SELECT country FROM customers WHERE name = 'Asha Rao') AND name != 'Asha Rao';"),
        ...
        Task("sc-t11", "Find product categories that contain a product priced over 80. Use a subquery.", "", "SELECT DISTINCT category FROM products WHERE category IN (SELECT category FROM products WHERE price > 80);"),
        Task("sc-t12", "Find customer IDs that appear in both the orders table AND the support_tickets table. Use INTERSECT.", "", "SELECT customer_id FROM orders INTERSECT SELECT customer_id FROM support_tickets;"),
        ...
        Task("sc-t17", "Find all support tickets assigned to orders placed by customers from 'India'. Use an IN subquery.", "", "SELECT id FROM support_tickets WHERE order_id IN (SELECT id FROM orders WHERE customer_id IN (SELECT id FROM customers WHERE country = 'India'));"),
        Task("sc-t18", "Create a single list containing all unique customer names AND all unique employee names. Use UNION.", "", "SELECT name FROM customers UNION SELECT name FROM employees;"),
    ...
    Lesson("ctes", 32, "Common Table Expressions", ["WITH", "readability"], ["customers", "orders", "order_items", "products", "payments"], [
            ...
            Task("l27-t6", "Interview level: use CTEs 'successful_payments' and 'customer_totals' to find customers whose successful payment total is above the average successful payment total. Return name and total_paid rounded to 2 decimals, sorted by total_paid descending.", "", "WITH successful_payments AS (SELECT o.customer_id, p.amount FROM payments p JOIN orders o ON o.id = p.order_id WHERE p.status = 'success'), customer_totals AS (SELECT c.id, c.name, SUM(sp.amount) AS total_paid FROM customers c JOIN successful_payments sp ON sp.customer_id = c.id GROUP BY c.id, c.name), avg_total AS (SELECT AVG(total_paid) AS avg_paid FROM customer_totals) SELECT name, ROUND(total_paid, 2) AS total_paid FROM customer_totals, avg_total WHERE total_paid > avg_paid ORDER BY total_paid DESC, name;"),
            Task("l27-t7", "Interview level: use CTEs to find each customer's latest successful payment. Return name, payment_id, amount, and created_at, sorted by name.", "", "WITH successful_payments AS (SELECT o.customer_id, p.id AS payment_id, p.amount, p.created_at FROM payments p JOIN orders o ON o.id = p.order_id WHERE p.status = 'success'), latest_payment_time AS (SELECT customer_id, MAX(created_at) AS latest_created_at FROM successful_payments GROUP BY customer_id) SELECT c.name, sp.payment_id, sp.amount, sp.created_at FROM successful_payments sp JOIN latest_payment_time lpt ON lpt.customer_id = sp.customer_id AND lpt.latest_created_at = sp.created_at JOIN customers c ON c.id = sp.customer_id ORDER BY c.name, sp.payment_id;"),
            Task("l27-t8", "Interview level: use CTEs to compute daily successful payment volume, then return only the days whose volume is above the average daily volume. Show payment_date and daily_volume rounded to 2 decimals, sorted by payment_date.", "", "WITH daily_volume AS (SELECT substr(created_at, 1, 10) AS payment_date, SUM(amount) AS daily_volume FROM payments WHERE status = 'success' GROUP BY payment_date), avg_volume AS (SELECT AVG(daily_volume) AS avg_daily_volume FROM daily_volume) SELECT payment_date, ROUND(daily_volume, 2) AS daily_volume FROM daily_volume, avg_volume WHERE daily_volume > avg_daily_volume ORDER BY payment_date;"),
        ]),
    Lesson("recursive-ctes", 33, "Recursive CTEs", ["WITH RECURSIVE", "hierarchies"], ["employees", "customers"], [
    ...
    Lesson("window-ranking", 34, "Window Ranking", ["ROW_NUMBER", "RANK", "PARTITION BY"], ["products", "customers", "orders", "order_items"], [
    ...
    Lesson("window-analytics", 35, "Running Totals and Moving Averages", ["SUM OVER", "AVG OVER"], ["orders", "order_items", "products"], [
    ...
    Lesson("window-row-number", 36, "Window ROW_NUMBER()", ["ROW_NUMBER", "payments", "first or latest"], ["payments", "customers", "orders"], [
            Task("wrn-t1", "Find the first successful payment for each order. Return order_id, payment_id, amount, and created_at, sorted by order_id.", "", "SELECT order_id, payment_id, amount, created_at FROM (SELECT order_id, id AS payment_id, amount, created_at, ROW_NUMBER() OVER (PARTITION BY order_id ORDER BY created_at ASC, id ASC) AS rn FROM payments WHERE status = 'success') WHERE rn = 1 ORDER BY order_id;"),
            Task("wrn-t2", "Find the latest transaction for each payment method. Return payment_method, payment_id, amount, status, and created_at, sorted by payment_method.", "", "SELECT payment_method, payment_id, amount, status, created_at FROM (SELECT payment_method, id AS payment_id, amount, status, created_at, ROW_NUMBER() OVER (PARTITION BY payment_method ORDER BY created_at DESC, id DESC) AS rn FROM payments) WHERE rn = 1 ORDER BY payment_method;"),
            Task("wrn-t3", "Find the top 2 largest payments made by each customer. Return customer name, amount, and status, sorted by name then amount descending.", "", "WITH ranked_payments AS (SELECT c.name, p.amount, p.status, ROW_NUMBER() OVER (PARTITION BY c.id ORDER BY p.amount DESC, p.id DESC) AS rn FROM customers c JOIN orders o ON o.customer_id = c.id JOIN payments p ON p.order_id = o.id) SELECT name, amount, status FROM ranked_payments WHERE rn <= 2 ORDER BY name, amount DESC, status;"),
            Task("wrn-t4", "Detect duplicate payment attempts for the same order and amount. Return order_id, amount, and duplicate_payment_id, sorted by order_id then duplicate_payment_id.", "", "SELECT order_id, amount, duplicate_payment_id FROM (SELECT order_id, amount, id AS duplicate_payment_id, ROW_NUMBER() OVER (PARTITION BY order_id, amount ORDER BY created_at ASC, id ASC) AS rn FROM payments) WHERE rn > 1 ORDER BY order_id, duplicate_payment_id;"),
            Task("wrn-t5", "Find the first 2 delivered orders for each customer. Return name, order_id, and ordered_at, sorted by name then ordered_at.", "", "WITH ranked_orders AS (SELECT c.name, o.id AS order_id, o.ordered_at, ROW_NUMBER() OVER (PARTITION BY c.id ORDER BY o.ordered_at ASC, o.id ASC) AS rn FROM customers c JOIN orders o ON o.customer_id = c.id WHERE o.status = 'delivered') SELECT name, order_id, ordered_at FROM ranked_orders WHERE rn <= 2 ORDER BY name, ordered_at, order_id;"),
        ]),
    Lesson("window-rank", 37, "Window RANK()", ["RANK", "ties", "payments"], ["payments", "customers", "orders", "products", "order_items", "employees"], [
            Task("wrk-t1", "Rank successful payments by amount within each payment method. Return payment_method, payment_id, amount, and payment_rank, sorted by payment_method then payment_rank.", "", "SELECT payment_method, id AS payment_id, amount, RANK() OVER (PARTITION BY payment_method ORDER BY amount DESC) AS payment_rank FROM payments WHERE status = 'success' ORDER BY payment_method, payment_rank, payment_id;"),
            Task("wrk-t2", "Rank failed or refunded payments globally by amount descending. Return payment_id, amount, status, and global_rank, sorted by global_rank then payment_id.", "", "SELECT id AS payment_id, amount, status, RANK() OVER (ORDER BY amount DESC) AS global_rank FROM payments WHERE status IN ('failed', 'refunded') ORDER BY global_rank, payment_id;"),
            Task("wrk-t3", "Rank customers globally by total successful payment spend. Return name, total_spent rounded to 2 decimals, and spend_rank, sorted by spend_rank then name.", "", "WITH customer_spend AS (SELECT c.id, c.name, SUM(p.amount) AS total_spent FROM customers c JOIN orders o ON o.customer_id = c.id JOIN payments p ON p.order_id = o.id WHERE p.status = 'success' GROUP BY c.id, c.name) SELECT name, ROUND(total_spent, 2) AS total_spent, RANK() OVER (ORDER BY total_spent DESC) AS spend_rank FROM customer_spend ORDER BY spend_rank, name;"),
            Task("wrk-t4", "Rank products by total refunded amount. Return product_id, name, refunded_amount rounded to 2 decimals, and refund_rank, sorted by refund_rank then product_id.", "", "WITH product_refunds AS (SELECT p.id, p.name, COALESCE(SUM(oi.quantity * oi.unit_price), 0.0) AS refunded_amount FROM products p JOIN order_items oi ON oi.product_id = p.id JOIN orders o ON o.id = oi.order_id JOIN payments pay ON pay.order_id = o.id WHERE pay.status = 'refunded' GROUP BY p.id, p.name) SELECT id AS product_id, name, ROUND(refunded_amount, 2) AS refunded_amount, RANK() OVER (ORDER BY refunded_amount DESC) AS refund_rank FROM product_refunds ORDER BY refund_rank, product_id;"),
            Task("wrk-t5", "Rank employees by salary within each team. Return team, name, salary, and salary_rank, sorted by team then salary_rank.", "", "SELECT team, name, salary, RANK() OVER (PARTITION BY team ORDER BY salary DESC) AS salary_rank FROM employees ORDER BY team, salary_rank, name;"),
        ]),
    Lesson("window-dense-rank", 38, "Window DENSE_RANK()", ["DENSE_RANK", "no gaps", "payments"], ["payments", "customers", "orders", "products", "order_items", "employees"], [
            Task("wdr-t1", "Dense-rank successful wallet payments by amount descending. Return payment_id, amount, and dense_rank, sorted by dense_rank then payment_id.", "", "SELECT id AS payment_id, amount, DENSE_RANK() OVER (ORDER BY amount DESC) AS dense_rank FROM payments WHERE payment_method = 'wallet' AND status = 'success' ORDER BY dense_rank, payment_id;"),
            Task("wdr-t2", "Dense-rank customers by their count of successful payment transactions. Return name, transaction_count, and tx_rank, sorted by tx_rank then name.", "", "WITH customer_transactions AS (SELECT c.id, c.name, COUNT(p.id) AS transaction_count FROM customers c JOIN orders o ON o.customer_id = c.id JOIN payments p ON p.order_id = o.id WHERE p.status = 'success' GROUP BY c.id, c.name) SELECT name, transaction_count, DENSE_RANK() OVER (ORDER BY transaction_count DESC) AS tx_rank FROM customer_transactions ORDER BY tx_rank, name;"),
            Task("wdr-t3", "Dense-rank payment dates by total successful payment volume. Return payment_date, daily_volume rounded to 2 decimals, and volume_rank, sorted by volume_rank then payment_date.", "", "WITH daily_volume AS (SELECT substr(created_at, 1, 10) AS payment_date, SUM(amount) AS daily_volume FROM payments WHERE status = 'success' GROUP BY payment_date) SELECT payment_date, ROUND(daily_volume, 2) AS daily_volume, DENSE_RANK() OVER (ORDER BY daily_volume DESC) AS volume_rank FROM daily_volume ORDER BY volume_rank, payment_date;"),
            Task("wdr-t4", "Dense-rank product categories by successful sales revenue. Return category, category_revenue rounded to 2 decimals, and revenue_rank, sorted by revenue_rank then category.", "", "WITH category_revenue AS (SELECT p.category, SUM(oi.quantity * oi.unit_price) AS category_revenue FROM products p JOIN order_items oi ON oi.product_id = p.id JOIN orders o ON o.id = oi.order_id JOIN payments pay ON pay.order_id = o.id WHERE pay.status = 'success' GROUP BY p.category) SELECT category, ROUND(category_revenue, 2) AS category_revenue, DENSE_RANK() OVER (ORDER BY category_revenue DESC) AS revenue_rank FROM category_revenue ORDER BY revenue_rank, category;"),
            Task("wdr-t5", "Dense-rank employees by hire year from oldest cohort to newest. Return name, hired_year, and cohort_rank, sorted by cohort_rank then name.", "", "SELECT name, strftime('%Y', hired_at) AS hired_year, DENSE_RANK() OVER (ORDER BY strftime('%Y', hired_at) ASC) AS cohort_rank FROM employees ORDER BY cohort_rank, name;"),
        ]),
    Lesson("window-lag", 39, "Window LAG()", ["LAG", "previous row", "payments"], ["payments", "customers", "orders", "support_tickets"], [
            Task("wlg-t1", "For each payment attempt, show the days since the previous attempt for the same order. Return order_id, payment_id, created_at, amount, and days_since_prev, sorted by order_id then payment_id.", "", "SELECT order_id, id AS payment_id, created_at, amount, ROUND(julianday(created_at) - julianday(LAG(created_at) OVER (PARTITION BY order_id ORDER BY created_at, id)), 4) AS days_since_prev FROM payments ORDER BY order_id, payment_id;"),
            Task("wlg-t2", "For each customer's successful payments, show the previous successful payment amount and the absolute difference. Return name, current_amount, prev_amount, and amount_diff, sorted by name then created_at.", "", "WITH customer_payments AS (SELECT c.id, c.name, p.amount, p.created_at, LAG(p.amount) OVER (PARTITION BY c.id ORDER BY p.created_at, p.id) AS prev_amount FROM customers c JOIN orders o ON o.customer_id = c.id JOIN payments p ON p.order_id = o.id WHERE p.status = 'success') SELECT name, amount AS current_amount, prev_amount, ROUND(ABS(amount - prev_amount), 2) AS amount_diff FROM customer_payments ORDER BY name, created_at;"),
            Task("wlg-t3", "Show the previous support ticket opened by each customer. Return name, ticket_id, opened_at, and prev_opened_at, sorted by name then opened_at.", "", "SELECT c.name, st.id AS ticket_id, st.opened_at, LAG(st.opened_at) OVER (PARTITION BY c.id ORDER BY st.opened_at, st.id) AS prev_opened_at FROM customers c JOIN support_tickets st ON st.customer_id = c.id ORDER BY c.name, st.opened_at, st.id;"),
            Task("wlg-t4", "Compare each day's successful payment volume to the previous day. Return payment_date, daily_volume, prev_day_volume, and volume_diff, sorted by payment_date.", "", "WITH daily_volume AS (SELECT substr(created_at, 1, 10) AS payment_date, SUM(amount) AS daily_volume FROM payments WHERE status = 'success' GROUP BY payment_date) SELECT payment_date, ROUND(daily_volume, 2) AS daily_volume, ROUND(LAG(daily_volume) OVER (ORDER BY payment_date), 2) AS prev_day_volume, ROUND(daily_volume - LAG(daily_volume) OVER (ORDER BY payment_date), 2) AS volume_diff FROM daily_volume ORDER BY payment_date;"),
            Task("wlg-t5", "Calculate days since the previous customer signup within the same country. Return country, name, signup_date, and days_since_last_signup, sorted by country then signup_date.", "", "SELECT country, name, signup_date, CAST(julianday(signup_date) - julianday(LAG(signup_date) OVER (PARTITION BY country ORDER BY signup_date, id)) AS INTEGER) AS days_since_last_signup FROM customers ORDER BY country, signup_date, id;"),
        ]),
    Lesson("window-lead", 40, "Window LEAD()", ["LEAD", "next row", "payments"], ["payments", "customers", "orders", "products", "shipments", "support_tickets"], [
            Task("wld-t1", "For each payment attempt, show the next attempt amount for the same order. Return order_id, payment_id, current_status, current_amount, and next_amount, sorted by order_id then payment_id.", "", "SELECT order_id, id AS payment_id, status AS current_status, amount AS current_amount, LEAD(amount) OVER (PARTITION BY order_id ORDER BY created_at, id) AS next_amount FROM payments ORDER BY order_id, payment_id;"),
            Task("wld-t2", "For each customer's successful payments, show the next payment method they use. Return name, payment_date, current_method, and next_method, sorted by name then payment_date.", "", "WITH customer_success AS (SELECT c.id, c.name, p.created_at, p.payment_method, LEAD(p.payment_method) OVER (PARTITION BY c.id ORDER BY p.created_at, p.id) AS next_method FROM customers c JOIN orders o ON o.customer_id = c.id JOIN payments p ON p.order_id = o.id WHERE p.status = 'success') SELECT name, created_at AS payment_date, payment_method AS current_method, next_method FROM customer_success ORDER BY name, payment_date;"),
            Task("wld-t3", "Compare each product price with the next more expensive product in the same category. Return category, name, price, and next_expensive_price, sorted by category then price.", "", "SELECT category, name, price, LEAD(price) OVER (PARTITION BY category ORDER BY price ASC, name ASC) AS next_expensive_price FROM products ORDER BY category, price, name;"),
            Task("wld-t4", "For each carrier, show the next shipping cost handled by that same carrier. Return carrier, order_id, shipping_cost, and next_shipping_cost, sorted by carrier then order_id.", "", "SELECT carrier, order_id, shipping_cost, LEAD(shipping_cost) OVER (PARTITION BY carrier ORDER BY order_id) AS next_shipping_cost FROM shipments ORDER BY carrier, order_id;"),
            Task("wld-t5", "For each customer support ticket, show the gap in days until their next ticket. Return name, ticket_id, opened_at, and days_to_next_ticket, sorted by name then opened_at.", "", "SELECT c.name, st.id AS ticket_id, st.opened_at, CAST(julianday(LEAD(st.opened_at) OVER (PARTITION BY c.id ORDER BY st.opened_at, st.id)) - julianday(st.opened_at) AS INTEGER) AS days_to_next_ticket FROM customers c JOIN support_tickets st ON st.customer_id = c.id ORDER BY c.name, st.opened_at, st.id;"),
        ]),
    Lesson("window-running-totals", 41, "Running Totals", ["SUM OVER", "COUNT OVER", "cumulative"], ["payments", "customers", "orders", "support_tickets"], [
            Task("wrt-t1", "Calculate the running total of successful payment volume within each payment method over time. Return payment_method, payment_id, created_at, amount, and running_volume, sorted by payment_method then created_at.", "", "SELECT payment_method, id AS payment_id, created_at, amount, ROUND(SUM(amount) OVER (PARTITION BY payment_method ORDER BY created_at, id), 2) AS running_volume FROM payments WHERE status = 'success' ORDER BY payment_method, created_at, payment_id;"),
            Task("wrt-t2", "Calculate the running total of successful customer spend. Return name, payment_id, created_at, amount, and running_spend, sorted by name then created_at.", "", "SELECT c.name, p.id AS payment_id, p.created_at, p.amount, ROUND(SUM(p.amount) OVER (PARTITION BY c.id ORDER BY p.created_at, p.id), 2) AS running_spend FROM customers c JOIN orders o ON o.customer_id = c.id JOIN payments p ON p.order_id = o.id WHERE p.status = 'success' ORDER BY c.name, p.created_at, p.id;"),
            Task("wrt-t3", "Compute cumulative revenue by payment date using successful payments only. Return payment_date, daily_volume, and cumulative_revenue, sorted by payment_date.", "", "WITH daily_volume AS (SELECT substr(created_at, 1, 10) AS payment_date, SUM(amount) AS daily_volume FROM payments WHERE status = 'success' GROUP BY payment_date) SELECT payment_date, ROUND(daily_volume, 2) AS daily_volume, ROUND(SUM(daily_volume) OVER (ORDER BY payment_date), 2) AS cumulative_revenue FROM daily_volume ORDER BY payment_date;"),
            Task("wrt-t4", "Show the running count of support tickets opened over time. Return ticket_id, opened_at, and running_ticket_count, sorted by opened_at then ticket_id.", "", "SELECT id AS ticket_id, opened_at, COUNT(*) OVER (ORDER BY opened_at, id) AS running_ticket_count FROM support_tickets ORDER BY opened_at, id;"),
            Task("wrt-t5", "Show the cumulative count of customers by signup date. Return name, signup_date, and cumulative_customers, sorted by signup_date then id.", "", "SELECT name, signup_date, COUNT(*) OVER (ORDER BY signup_date, id) AS cumulative_customers FROM customers ORDER BY signup_date, id;"),
        ]),
    Lesson("top-n-per-group", 42, "Top N Per Group", ["ROW_NUMBER", "filter ranked rows"], ["products", "orders", "order_items"], [
    ...
    Lesson("advanced-capstone", 43, "Advanced Queries: Interview Capstone", ["Hint-Free", "CTEs", "Windows"], ["customers", "orders", "products"], [
        ...
        Task("adc-t17", "Show each order ID alongside the next order ID (ordered by id). Use a CTE with LEAD. Return id and next_id.", "", "WITH OrderSeq AS (SELECT id, LEAD(id) OVER(ORDER BY id) AS next_id FROM orders) SELECT id, next_id FROM OrderSeq WHERE next_id IS NOT NULL;"),
    ...
    Lesson("execution-order", 44, "Query Order", ["WHERE", "GROUP BY", "HAVING", "ORDER BY"], ["orders", "order_items", "customers"], [
    ...
    Lesson("indexes-query-plans", 45, "Indexes and Query Plans", ["CREATE INDEX", "EXPLAIN QUERY PLAN"], ["products", "orders", "customers", "order_items"], [
    ...
    Lesson("interview-filters", 46, "Interview Filtering Patterns", ["BETWEEN", "NOT IN", "multi-condition"], ["products", "customers", "orders"], [
    ...
    Lesson("insert", 47, "INSERT Rows", ["INSERT"], ["products", "customers", "orders", "order_items"], [
    ...
    Lesson("update", 48, "UPDATE Rows", ["UPDATE", "SET"], ["products", "orders", "support_tickets"], [
    ...
    Lesson("delete", 49, "DELETE Rows", ["DELETE"], ["products", "orders", "support_tickets"], [
    ...
    Lesson("create-table", 50, "CREATE TABLE", ["CREATE TABLE", "types"], ["products"], [
    ...
    Lesson("alter-table", 51, "ALTER TABLE", ["ALTER TABLE"], ["products", "customers"], [
    ...
    Lesson("drop-table", 52, "DROP TABLE", ["DROP TABLE", "IF EXISTS"], ["support_tickets", "shipments"], [
        ...
        Task("l18-t5", "Create a table named temp_logs, insert two rows into it, then truncate the table so the structure remains but all rows are removed.", "", "CREATE TABLE temp_logs (id INTEGER PRIMARY KEY, note TEXT); INSERT INTO temp_logs (id, note) VALUES (1, 'a'), (2, 'b'); TRUNCATE TABLE temp_logs;", "SELECT COUNT(*) AS row_count FROM temp_logs;"),
    ]),
    Lesson("insert-select", 53, "CREATE AS & INSERT SELECT", ["CREATE TABLE AS", "INSERT INTO SELECT"], ["orders", "products"], [
    ...
    Lesson("constraints-not-null-default", 54, "Constraints: NOT NULL & DEFAULT", ["NOT NULL", "DEFAULT", "CREATE TABLE"], [], [
    ...
    Lesson("constraints-unique-check", 55, "Constraints: UNIQUE & CHECK", ["UNIQUE", "CHECK"], [], [
    ...
    Lesson("constraints-keys", 56, "Constraints: Keys", ["PRIMARY KEY", "FOREIGN KEY", "AUTOINCREMENT"], ["customers"], [
    ...
    Lesson("foreign-keys", 57, "Foreign Keys", ["FOREIGN KEY", "ON DELETE", "CASCADE", "RESTRICT", "SET NULL"], ["customers", "orders", "products", "order_items", "shipments", "employees", "support_tickets"], [
        Task("fk-t1", "Create a table 'customer_preferences' with columns 'customer_id' (INTEGER) and 'theme' (TEXT). Add a FOREIGN KEY constraint on 'customer_id' referencing 'customers(id)' ON DELETE CASCADE.", "", "CREATE TABLE customer_preferences (customer_id INTEGER, theme TEXT, FOREIGN KEY(customer_id) REFERENCES customers(id) ON DELETE CASCADE);", "SELECT sql FROM sqlite_master WHERE type='table' AND name='customer_preferences';"),
        Task("fk-t2", "Create a table 'loyalty_rewards' with columns 'reward_id' (INTEGER PRIMARY KEY), 'customer_id' (INTEGER), and 'points' (INTEGER). Add a FOREIGN KEY constraint on 'customer_id' referencing 'customers(id)' ON DELETE RESTRICT.", "", "CREATE TABLE loyalty_rewards (reward_id INTEGER PRIMARY KEY, customer_id INTEGER, points INTEGER, FOREIGN KEY(customer_id) REFERENCES customers(id) ON DELETE RESTRICT);", "SELECT sql FROM sqlite_master WHERE type='table' AND name='loyalty_rewards';"),
        Task("fk-t3", "Create a table 'employee_project_assignments' with columns 'employee_id' (INTEGER) and 'project_id' (INTEGER). Define both columns together as a composite PRIMARY KEY. Add a foreign key constraint on 'employee_id' referencing 'employees(id)' ON DELETE CASCADE.", "", "CREATE TABLE employee_project_assignments (employee_id INTEGER, project_id INTEGER, PRIMARY KEY(employee_id, project_id), FOREIGN KEY(employee_id) REFERENCES employees(id) ON DELETE CASCADE);", "SELECT sql FROM sqlite_master WHERE type='table' AND name='employee_project_assignments';"),
        Task("fk-t4", "Create a table 'customer_api_keys' with columns 'key_id' (INTEGER PRIMARY KEY), 'customer_id' (INTEGER), and 'api_key' (TEXT UNIQUE). Add a foreign key constraint on 'customer_id' referencing 'customers(id)' ON DELETE CASCADE.", "", "CREATE TABLE customer_api_keys (key_id INTEGER PRIMARY KEY, customer_id INTEGER, api_key TEXT UNIQUE, FOREIGN KEY(customer_id) REFERENCES customers(id) ON DELETE CASCADE);", "SELECT sql FROM sqlite_master WHERE type='table' AND name='customer_api_keys';"),
        Task("fk-t5", "Create a table 'support_notes' with columns 'note_id' (INTEGER PRIMARY KEY), 'ticket_id' (INTEGER), and 'content' (TEXT). Add a foreign key constraint on 'ticket_id' referencing 'support_tickets(id)' ON DELETE SET NULL.", "", "CREATE TABLE support_notes (note_id INTEGER PRIMARY KEY, ticket_id INTEGER, content TEXT, FOREIGN KEY(ticket_id) REFERENCES support_tickets(id) ON DELETE SET NULL);", "SELECT sql FROM sqlite_master WHERE type='table' AND name='support_notes';"),
        Task("fk-t6", "Create a table 'categories_tree' with columns 'id' (INTEGER PRIMARY KEY), 'name' (TEXT NOT NULL), and 'parent_id' (INTEGER). Add a foreign key constraint on 'parent_id' referencing 'categories_tree(id)' ON DELETE CASCADE.", "", "CREATE TABLE categories_tree (id INTEGER PRIMARY KEY, name TEXT NOT NULL, parent_id INTEGER, FOREIGN KEY(parent_id) REFERENCES categories_tree(id) ON DELETE CASCADE);", "SELECT sql FROM sqlite_master WHERE type='table' AND name='categories_tree';"),
        Task("fk-t7", "Identify products that have never been ordered (have no referencing rows in the 'order_items' table). Return the product 'id' and 'name', sorted by 'id'.", "", "SELECT id, name FROM products WHERE id NOT IN (SELECT DISTINCT product_id FROM order_items) ORDER BY id;"),
        Task("fk-t8", "Find all employees who have a manager. Return the employee name (as 'employee') and their manager's name (as 'manager'), sorted by employee name.", "", "SELECT e1.name AS employee, e2.name AS manager FROM employees e1 JOIN employees e2 ON e1.manager_id = e2.id ORDER BY employee;"),
        Task("fk-t9", "For each customer, count the number of orders they have placed. Return the customer 'id', 'name', and order count (as 'order_count'), sorted by 'order_count' descending.", "", "SELECT customers.id, customers.name, COUNT(orders.id) AS order_count FROM customers LEFT JOIN orders ON orders.customer_id = customers.id GROUP BY customers.id, customers.name ORDER BY order_count DESC;"),
        Task("fk-t10", "Find all support tickets that reference a valid order. Return the ticket 'id' and the referenced 'order_id', sorted by 'order_id'.", "", "SELECT id, order_id FROM support_tickets WHERE order_id IS NOT NULL ORDER BY order_id;"),
    ]),
    Lesson("views", 58, "Database Views", ["CREATE VIEW", "DROP VIEW"], ["orders", "order_items", "products"], [
    ...
    Lesson("indexes", 59, "Indexes", ["CREATE INDEX", "UNIQUE INDEX", "DROP INDEX"], ["customers", "orders"], [
    ...
    Lesson("injection", 60, "Security: SQL Injection", ["hacker mode", "authentication bypass", "UNION payloads"], ["customers"], [
        ...
        Task("l47-t3", "Hacker Mode: Show what happens if someone types `' OR '1'='1` into a name search field. The vulnerable query is: SELECT * FROM customers WHERE name = '[INPUT]'.", "", "SELECT * FROM customers WHERE name = '' OR '1'='1';", ""),
    ]),
    Lesson("ddl-capstone", 61, "DDL & Admin: Interview Capstone", ["Hint-Free", "Data Modeling"], ["customers", "orders", "products"], [
        ...
        Task("dc-t17", "Hacker Mode: Bypass an authentication query. Write the resulting query if you inject `' OR 1=1 --` into the name field of `SELECT * FROM customers WHERE name = '[INPUT]'`.", "", "SELECT * FROM customers WHERE name = '' OR 1=1 --;", "SELECT 1;"),
    ...
    Lesson("leetcode-patterns", 62, "Classic Interview Patterns", ["second highest", "duplicates", "gaps"], ["products", "customers", "orders"], [
    ...
    Lesson("business-analytics", 63, "Business Intelligence SQL", ["JOIN", "arithmetic", "business-logic"], ["customers", "orders", "order_items", "products", "shipments"], [
    ...
    Lesson("final-interview-sets", 64, "Final Interview Sets", ["multi-step", "analytics", "business SQL"], ["customers", "orders", "order_items", "products", "shipments", "support_tickets"], [
    ...
    Lesson("mixed-bag-capstone", 65, "Mixed Bag: Ultimate Capstone", ["Hint-Free", "Advanced Business Logic"], ["customers", "orders", "order_items", "products", "shipments", "employees", "support_tickets"], [
        ...
        Task("mbc-t7", "Find the customer who has opened the most support tickets. Return name and ticket_count.", "", "SELECT customers.name, COUNT(support_tickets.id) AS ticket_count FROM customers JOIN support_tickets ON support_tickets.customer_id = customers.id GROUP BY customers.id, customers.name ORDER BY ticket_count DESC LIMIT 1;"),
        Task("mbc-t8", "Identify orders that contain more than 1 distinct product category. Return order_id and category_count.", "", "SELECT order_items.order_id, COUNT(DISTINCT products.category) AS category_count FROM order_items JOIN products ON products.id = order_items.product_id GROUP BY order_items.order_id HAVING COUNT(DISTINCT products.category) > 1;"),
        ...
        Task("mbc-t14", "List employees who manage exactly one other person. Return employee name and report_count.", "", "SELECT m.name, COUNT(e.id) AS report_count FROM employees m JOIN employees e ON e.manager_id = m.id GROUP BY m.id, m.name HAVING COUNT(e.id) = 1;"),
        ...
        Task("mbc-t21", "Find the top 3 customers who have the highest ratio of discounted items to total items purchased. Return name and discount_ratio.", "", "WITH ItemCounts AS (SELECT customers.name, SUM(CASE WHEN order_items.unit_price < products.price THEN 1 ELSE 0 END) AS discount_items, COUNT(order_items.id) AS total_items FROM customers JOIN orders ON orders.customer_id = customers.id JOIN order_items ON order_items.order_id = orders.id JOIN products ON products.id = order_items.product_id GROUP BY customers.id, customers.name) SELECT name, CAST(discount_items AS REAL) / total_items AS discount_ratio FROM ItemCounts ORDER BY discount_ratio DESC LIMIT 3;"),
        Task("mbc-t22", "Calculate the absolute difference in average lifetime spend between customers from 'USA' and 'UK'. Return the absolute difference as 'spend_diff'.", "", "WITH CustSpend AS (SELECT customers.country, customers.id, SUM(order_items.quantity * order_items.unit_price) AS total_spend FROM customers JOIN orders ON orders.customer_id = customers.id JOIN order_items ON order_items.order_id = orders.id WHERE customers.country IN ('USA', 'UK') GROUP BY customers.id, customers.country), AvgSpend AS (SELECT country, AVG(total_spend) AS avg_s FROM CustSpend GROUP BY country) SELECT ABS(MAX(avg_s) - MIN(avg_s)) AS spend_diff FROM AvgSpend;"),
        Task("mbc-t23", "Identify products that were ordered on consecutive days by the same customer. Return customer name and product name.", "", "WITH OrderDates AS (SELECT customers.name AS cust_name, products.name AS prod_name, strftime('%Y-%m-%d', orders.ordered_at) AS date_str FROM customers JOIN orders ON orders.customer_id = customers.id JOIN order_items ON order_items.order_id = orders.id JOIN products ON products.id = order_items.product_id GROUP BY customers.name, products.name, strftime('%Y-%m-%d', orders.ordered_at)), Lagged AS (SELECT cust_name, prod_name, date_str, LAG(date_str) OVER(PARTITION BY cust_name, prod_name ORDER BY date_str) AS prev_date FROM OrderDates) SELECT DISTINCT cust_name, prod_name FROM Lagged WHERE julianday(date_str) - julianday(prev_date) = 1;"),
        Task("mbc-t24", "Find the most common shipping carrier used for orders that contain 'Electronics'. Return carrier and count.", "", "SELECT shipments.carrier, COUNT(DISTINCT orders.id) AS count FROM shipments JOIN orders ON orders.id = shipments.order_id JOIN order_items ON order_items.order_id = orders.id JOIN products ON products.id = order_items.product_id WHERE products.category = 'Electronics' GROUP BY shipments.carrier ORDER BY count DESC LIMIT 1;"),
        Task("mbc-t25", "Assuming every resolved support ticket saves the company $50, calculate the total money saved by each employee. Return employee name and money_saved.", "", "SELECT employees.name, COUNT(support_tickets.id) * 50 AS money_saved FROM employees JOIN support_tickets ON support_tickets.employee_id = employees.id WHERE support_tickets.status = 'resolved' GROUP BY employees.id, employees.name;"),
        Task("mbc-t26", "Find customers who have placed at least one order in every single distinct month that exists in the orders table. Return customer name.", "", "WITH TotalMonths AS (SELECT COUNT(DISTINCT substr(ordered_at, 1, 7)) AS num_months FROM orders), CustMonths AS (SELECT customer_id, COUNT(DISTINCT substr(ordered_at, 1, 7)) AS cust_months FROM orders GROUP BY customer_id) SELECT customers.name FROM customers JOIN CustMonths ON CustMonths.customer_id = customers.id WHERE CustMonths.cust_months = (SELECT num_months FROM TotalMonths);"),
        Task("mbc-t27", "Identify the product category that has the highest variance in product prices (Max price - Min price). Return category and price_variance.", "", "SELECT category, MAX(price) - MIN(price) AS price_variance FROM products GROUP BY category ORDER BY price_variance DESC LIMIT 1;"),
        Task("mbc-t28", "List all employees who share the exact same manager and have resolved the exact same number of tickets. Return e1_name and e2_name.", "", "WITH TicketCounts AS (SELECT employee_id, COUNT(id) AS resolved_count FROM support_tickets WHERE status = 'resolved' GROUP BY employee_id) SELECT e1.name AS e1_name, e2.name AS e2_name FROM employees e1 JOIN employees e2 ON e1.manager_id = e2.manager_id AND e1.id < e2.id JOIN TicketCounts t1 ON t1.employee_id = e1.id JOIN TicketCounts t2 ON t2.employee_id = e2.id WHERE t1.resolved_count = t2.resolved_count;"),
        Task("mbc-t29", "Find the customer who has the longest gap (in days) between any two consecutive orders. Return customer name and max_gap.", "", "WITH Gaps AS (SELECT customer_id, julianday(ordered_at) - julianday(LAG(ordered_at) OVER(PARTITION BY customer_id ORDER BY ordered_at)) AS gap FROM orders) SELECT customers.name, MAX(Gaps.gap) AS max_gap FROM customers JOIN Gaps ON Gaps.customer_id = customers.id WHERE Gaps.gap IS NOT NULL GROUP BY customers.id, customers.name ORDER BY max_gap DESC LIMIT 1;"),
        Task("mbc-t30", "Calculate the percentage of total company revenue contributed by the top 10% of customers (by spend).", "", "WITH Spend AS (SELECT customer_id, SUM(quantity * unit_price) AS total FROM order_items JOIN orders ON orders.id = order_items.order_id GROUP BY customer_id), Ranked AS (SELECT total, NTILE(10) OVER(ORDER BY total DESC) as percentile FROM Spend) SELECT ROUND(SUM(CASE WHEN percentile = 1 THEN total ELSE 0 END) * 100.0 / SUM(total), 2) AS top_10_pct FROM Ranked;"),
        Task("mbc-t31", "Identify the product that is most frequently bought alongside 'MacBook Pro' in the same order. Return the product name.", "", "SELECT p2.name FROM order_items o1 JOIN products p1 ON p1.id = o1.product_id JOIN order_items o2 ON o2.order_id = o1.order_id AND o2.product_id != o1.product_id JOIN products p2 ON p2.id = o2.product_id WHERE p1.name = 'MacBook Pro' GROUP BY p2.id, p2.name ORDER BY COUNT(*) DESC LIMIT 1;"),
        Task("mbc-t32", "Find the average number of items per order for customers who have never used a referral link (referrer_id IS NULL).", "", "WITH ItemCounts AS (SELECT orders.id, SUM(order_items.quantity) AS qty FROM orders JOIN order_items ON order_items.order_id = orders.id JOIN customers ON customers.id = orders.customer_id WHERE customers.referrer_id IS NULL GROUP BY orders.id) SELECT AVG(qty) FROM ItemCounts;"),
        Task("mbc-t33", "Determine which day of the week (0-6) generates the highest average revenue per order. Return day_of_week and avg_revenue.", "", "WITH OrderRevs AS (SELECT orders.id, strftime('%w', orders.ordered_at) AS day_of_week, SUM(order_items.quantity * order_items.unit_price) AS rev FROM orders JOIN order_items ON order_items.order_id = orders.id GROUP BY orders.id, day_of_week) SELECT day_of_week, AVG(rev) AS avg_revenue FROM OrderRevs GROUP BY day_of_week ORDER BY avg_revenue DESC LIMIT 1;"),
        Task("mbc-t34", "Find customers whose total spending is strictly greater than the average spending of all customers from their own country. Return customer name.", "", "WITH CustSpend AS (SELECT customers.id, customers.name, customers.country, SUM(order_items.quantity * order_items.unit_price) AS spend FROM customers JOIN orders ON orders.customer_id = customers.id JOIN order_items ON order_items.order_id = orders.id GROUP BY customers.id, customers.name, customers.country), CountryAvg AS (SELECT country, AVG(spend) AS avg_s FROM CustSpend GROUP BY country) SELECT CustSpend.name FROM CustSpend JOIN CountryAvg ON CountryAvg.country = CustSpend.country WHERE CustSpend.spend > CountryAvg.avg_s;"),
        Task("mbc-t35", "Identify orders that were shipped faster than the average shipping time for their specific carrier. Return order id.", "", "WITH ShipTimes AS (SELECT shipments.order_id, shipments.carrier, julianday(shipments.shipped_at) - julianday(orders.ordered_at) AS ship_time FROM shipments JOIN orders ON orders.id = shipments.order_id), CarrierAvg AS (SELECT carrier, AVG(ship_time) AS avg_t FROM ShipTimes GROUP BY carrier) SELECT ShipTimes.order_id FROM ShipTimes JOIN CarrierAvg ON CarrierAvg.carrier = ShipTimes.carrier WHERE ShipTimes.ship_time < CarrierAvg.avg_t;"),
        Task("mbc-t36", "List the names of customers who have ordered every single product in the 'Books' category. Use COUNT DISTINCT.", "", "SELECT customers.name FROM customers JOIN orders ON orders.customer_id = customers.id JOIN order_items ON order_items.order_id = orders.id JOIN products ON products.id = order_items.product_id WHERE products.category = 'Books' GROUP BY customers.id, customers.name HAVING COUNT(DISTINCT products.id) = (SELECT COUNT(id) FROM products WHERE category = 'Books');"),
        Task("mbc-t37", "Find the employee with the highest ratio of resolved to unresolved support tickets. Exclude employees with 0 unresolved tickets.", "", "WITH Stats AS (SELECT employee_id, SUM(CASE WHEN status = 'resolved' THEN 1 ELSE 0 END) AS res, SUM(CASE WHEN status != 'resolved' THEN 1 ELSE 0 END) AS unres FROM support_tickets GROUP BY employee_id) SELECT employees.name FROM employees JOIN Stats ON Stats.employee_id = employees.id WHERE Stats.unres > 0 ORDER BY CAST(Stats.res AS REAL) / Stats.unres DESC LIMIT 1;"),
        Task("mbc-t38", "Determine the most popular product category (by total quantity sold) for each country. Return country and category.", "", "WITH CatSales AS (SELECT customers.country, products.category, SUM(order_items.quantity) AS qty FROM customers JOIN orders ON orders.customer_id = customers.id JOIN order_items ON order_items.order_id = orders.id JOIN products ON products.id = order_items.product_id GROUP BY customers.country, products.category), Ranked AS (SELECT country, category, ROW_NUMBER() OVER(PARTITION BY country ORDER BY qty DESC) AS rnk FROM CatSales) SELECT country, category FROM Ranked WHERE rnk = 1;"),
        Task("mbc-t39", "Identify customers who have spent exactly the same total amount as at least one other customer. Return name and total_spend.", "", "WITH Spend AS (SELECT customers.id, customers.name, SUM(order_items.quantity * order_items.unit_price) AS total FROM customers JOIN orders ON orders.customer_id = customers.id JOIN order_items ON order_items.order_id = orders.id GROUP BY customers.id, customers.name) SELECT s1.name, s1.total FROM Spend s1 JOIN Spend s2 ON s1.total = s2.total AND s1.id != s2.id;"),
        Task("mbc-t40", "Find the product that took the longest to sell its first unit after the store opened (first ever order date). Return name.", "", "WITH FirstSales AS (SELECT product_id, MIN(orders.ordered_at) as first_sale FROM order_items JOIN orders ON orders.id = order_items.order_id GROUP BY product_id), StoreOpen AS (SELECT MIN(ordered_at) AS open_date FROM orders) SELECT products.name FROM products JOIN FirstSales ON FirstSales.product_id = products.id CROSS JOIN StoreOpen ORDER BY julianday(FirstSales.first_sale) - julianday(StoreOpen.open_date) DESC LIMIT 1;"),
        Task("mbc-t41", "List customers who have never ordered a product that costs more than 50. Return name.", "", "SELECT name FROM customers WHERE id NOT IN (SELECT orders.customer_id FROM orders JOIN order_items ON order_items.order_id = orders.id JOIN products ON products.id = order_items.product_id WHERE products.price > 50) AND id IN (SELECT customer_id FROM orders);"),
        Task("mbc-t42", "Determine the average order value for orders that contain at least one discounted item, versus orders with NO discounted items. Return 'has_discount' (1 or 0) and 'avg_order_value'.", "", "WITH OrderFlags AS (SELECT orders.id, MAX(CASE WHEN order_items.unit_price < products.price THEN 1 ELSE 0 END) AS has_discount, SUM(order_items.quantity * order_items.unit_price) AS total FROM orders JOIN order_items ON order_items.order_id = orders.id JOIN products ON products.id = order_items.product_id GROUP BY orders.id) SELECT has_discount, AVG(total) AS avg_order_value FROM OrderFlags GROUP BY has_discount;"),
        Task("mbc-t43", "Find any employee who was assigned a support ticket from a customer they personally referred. Return employee name.", "", "SELECT DISTINCT employees.name FROM employees JOIN customers ON customers.referrer_id = (SELECT id FROM customers c2 WHERE c2.email = employees.email) JOIN support_tickets ON support_tickets.customer_id = customers.id WHERE support_tickets.employee_id = employees.id;"),
        Task("mbc-t44", "Calculate the year-over-year growth in the number of unique customers placing orders. Return year and yoy_diff.", "", "WITH YearlyCusts AS (SELECT strftime('%Y', ordered_at) AS year, COUNT(DISTINCT customer_id) AS custs FROM orders GROUP BY year) SELECT year, custs - LAG(custs) OVER(ORDER BY year) AS yoy_diff FROM YearlyCusts;"),
        Task("mbc-t45", "Identify the customer who has ordered the highest variety of distinct products. Return name and distinct_products.", "", "SELECT customers.name, COUNT(DISTINCT order_items.product_id) AS distinct_products FROM customers JOIN orders ON orders.customer_id = customers.id JOIN order_items ON order_items.order_id = orders.id GROUP BY customers.id, customers.name ORDER BY distinct_products DESC LIMIT 1;"),
        Task("mbc-t46", "Find products that have been ordered by customers from more than 3 different countries. Return name.", "", "SELECT products.name FROM products JOIN order_items ON order_items.product_id = products.id JOIN orders ON orders.id = order_items.order_id JOIN customers ON customers.id = orders.customer_id GROUP BY products.id, products.name HAVING COUNT(DISTINCT customers.country) > 3;"),
        Task("mbc-t47", "Calculate the median price of products in the 'Electronics' category. Use NTILE(2) or ROW_NUMBER logic.", "", "WITH Ranked AS (SELECT price, ROW_NUMBER() OVER(ORDER BY price) AS rnk, COUNT(*) OVER() AS total FROM products WHERE category = 'Electronics') SELECT AVG(price) FROM Ranked WHERE rnk IN (total/2 + 1, (total+1)/2);"),
        Task("mbc-t48", "List the email addresses of employees who have resolved tickets for customers in 'Canada'.", "", "SELECT DISTINCT employees.email FROM employees JOIN support_tickets ON support_tickets.employee_id = employees.id JOIN customers ON customers.id = support_tickets.customer_id WHERE support_tickets.status = 'resolved' AND customers.country = 'Canada';"),
        Task("mbc-t49", "Find the month where the store had the highest ratio of delayed shipments (delivered_at > shipped_at + 2 days) to total shipments.", "", "WITH MonthlyShip AS (SELECT substr(shipped_at, 1, 7) AS month, SUM(CASE WHEN julianday(delivered_at) - julianday(shipped_at) > 2 THEN 1 ELSE 0 END) AS delayed, COUNT(*) AS total FROM shipments WHERE shipped_at IS NOT NULL GROUP BY month) SELECT month FROM MonthlyShip ORDER BY CAST(delayed AS REAL) / total DESC LIMIT 1;"),
        Task("mbc-t50", "Identify the customer whose most expensive single order item (quantity * unit_price) accounts for more than 80% of their entire lifetime spend. Return name.", "", "WITH ItemTotals AS (SELECT orders.customer_id, order_items.quantity * order_items.unit_price AS item_val FROM orders JOIN order_items ON order_items.order_id = orders.id), CustSpend AS (SELECT customer_id, MAX(item_val) AS max_item, SUM(item_val) AS total_spend FROM ItemTotals GROUP BY customer_id) SELECT customers.name FROM CustSpend JOIN customers ON customers.id = CustSpend.customer_id WHERE max_item > (0.8 * total_spend);"),
    ]),
]
...
NOTES_BY_LESSON = {
    ...
    "foreign-keys-mcq": "https://www.w3schools.com/sql/sql_foreignkey.asp",
    "subqueries": "https://mode.com/sql-tutorial/sql-subqueries/",
    ...
    "window-row-number": "https://www.postgresql.org/docs/current/tutorial-window.html",
    "window-rank": "https://www.postgresql.org/docs/current/functions-window.html",
    "window-dense-rank": "https://www.postgresql.org/docs/current/functions-window.html",
    "window-lag": "https://www.postgresql.org/docs/current/functions-window.html",
    "window-lead": "https://www.postgresql.org/docs/current/functions-window.html",
    "window-running-totals": "https://www.postgresql.org/docs/current/functions-window.html",
...
def build_connection() -> sqlite3.Connection:
    ...
    conn.executemany("INSERT INTO payments VALUES (?, ?, ?, ?, ?, ?)", SEED["payments"])
    conn.commit()
    return conn
...
def execute_sql(conn: sqlite3.Connection, sql: str) -> dict[str, Any]:
    ...
    normalized = [normalize_sqlite_statement(statement) for statement in statements]
...
def normalize_sqlite_statement(statement: str) -> str:
    truncate_match = re.match(
        r"^\s*TRUNCATE\s+TABLE\s+([A-Za-z_][A-Za-z0-9_]*)\s*$",
        statement,
        re.IGNORECASE,
    )
    if truncate_match:
...
GROUP_BY_LESSON = {
    ...
    "subqueries": "Sets & Subqueries",
    
    "ctes": "Advanced Queries",
    "recursive-ctes": "Advanced Queries",
    "window-ranking": "Advanced Queries",
    "window-analytics": "Advanced Queries",
    "window-row-number": "Advanced Queries",
    "window-rank": "Advanced Queries",
    "window-dense-rank": "Advanced Queries",
    "window-lag": "Advanced Queries",
    "window-lead": "Advanced Queries",
    "window-running-totals": "Advanced Queries",
    ...
    "foreign-keys": "DDL & Admin",
    "foreign-keys-mcq": "DDL & Admin",
    "views": "DDL & Admin",
    "indexes": "DDL & Admin",
    "injection": "DDL & Admin",
