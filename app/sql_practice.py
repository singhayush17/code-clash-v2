...
LESSONS = [
    ...
    Lesson("outer-joins", 7, "LEFT JOINs", ["LEFT JOIN", "missing rows"], ["products", "order_items", "orders", "shipments"], [
        ...
        Task("l7-t5", "Use a LEFT JOIN to find customers who have NEVER placed an order. Show name, sorted by name.", "", "SELECT customers.name FROM customers LEFT JOIN orders ON orders.customer_id = customers.id WHERE orders.id IS NULL ORDER BY customers.name;"),
    ]),
    Lesson("right-joins", 8, "RIGHT JOINs", ["RIGHT JOIN", "driving tables"], ["order_items", "products"], [
        Task("l8r-t1", "Show every product name and any order item id (as 'item_id') that references it. Drive the query from order_items on the left and RIGHT JOIN to products on the right. Sort by product name.", "", "SELECT products.name, order_items.id AS item_id FROM order_items RIGHT JOIN products ON products.id = order_items.product_id ORDER BY products.name;"),
        Task("l8r-t2", "Find products that have never been ordered using a RIGHT JOIN (order_items on left, products on right). Show name, sorted by name.", "", "SELECT products.name FROM order_items RIGHT JOIN products ON products.id = order_items.product_id WHERE order_items.id IS NULL ORDER BY products.name;"),
        Task("l8r-t3", "Show all employees and their manager's name (as 'manager'). Use a RIGHT JOIN where managers are on the left and employees are on the right. Sort by employee name.", "", "SELECT e.name, m.name AS manager FROM employees m RIGHT JOIN employees e ON e.manager_id = m.id ORDER BY e.name;"),
        Task("l8r-t4", "Count how many order items exist for each product using a RIGHT JOIN (order_items left, products right). Show product name and 'item_count', sorted by item_count descending.", "", "SELECT products.name, COUNT(order_items.id) AS item_count FROM order_items RIGHT JOIN products ON products.id = order_items.product_id GROUP BY products.id, products.name ORDER BY item_count DESC;"),
        Task("l8r-t5", "Find customers with no support tickets using a RIGHT JOIN. Support tickets on the left, customers on the right. Show customer name, sorted by name.", "", "SELECT customers.name FROM support_tickets RIGHT JOIN customers ON customers.id = support_tickets.customer_id WHERE support_tickets.id IS NULL ORDER BY customers.name;"),
    ]),
    Lesson("full-joins", 9, "FULL JOINs", ["FULL OUTER JOIN", "mismatches"], ["customers", "orders", "shipments"], [
        Task("l9f-t1", "Show all customers and all orders. If a customer has no orders, or an order has no matching customer (due to bad data), show them anyway. Select customer name and order id, sorted by customer name. Use FULL JOIN.", "", "SELECT customers.name, orders.id FROM customers FULL JOIN orders ON orders.customer_id = customers.id ORDER BY customers.name;"),
        Task("l9f-t2", "Find completely unmatched records: customers without orders AND orders without matching customers. Show customer name and order id. Sort by order id. Use FULL JOIN.", "", "SELECT customers.name, orders.id FROM customers FULL JOIN orders ON orders.customer_id = customers.id WHERE customers.id IS NULL OR orders.id IS NULL ORDER BY orders.id;"),
        Task("l9f-t3", "Show all orders and all shipments together. Return order id, shipment id, and status, sorted by order id. Use FULL JOIN.", "", "SELECT orders.id, shipments.id, orders.status FROM orders FULL JOIN shipments ON shipments.order_id = orders.id ORDER BY orders.id;"),
        ...
        Task("l9f-t5", "List all employees and all support tickets. This makes no logical sense but do it anyway using a FULL JOIN on employee id = ticket id. Show employee name and ticket id, sorted by employee name.", "", "SELECT employees.name, support_tickets.id FROM employees FULL JOIN support_tickets ON support_tickets.id = employees.id ORDER BY employees.name;"),
    ]),
    Lesson("self-joins", 10, "Self Joins", ["self join", "hierarchies"], ["employees", "customers"], [
    ...
    Lesson("semi-anti-joins", 11, "EXISTS and Anti-Joins", ["EXISTS", "NOT EXISTS"], ["customers", "orders", "products", "order_items", "shipments"], [
    ...
    Lesson("join-patterns", 12, "Join Interview Scenarios", ["identify the join"], ["customers", "orders", "products"], [
    ...
    Lesson("nulls", 13, "Working with NULL", ["IS NULL", "COALESCE"], ["customers", "employees", "shipments"], [
    ...
    Lesson("expressions", 14, "Expressions", ["AS", "CASE", "calculation"], ["order_items", "products", "employees"], [
    ...
    Lesson("string-functions", 15, "String Functions", ["LOWER", "SUBSTR", "LENGTH"], ["customers", "products"], [
    ...
    Lesson("date-analytics", 16, "Date Analytics", ["strftime", "julianday", "cohorts"], ["customers", "orders", "shipments"], [
    ...
    Lesson("data-cleaning", 17, "Data Cleaning Queries", ["TRIM", "COALESCE", "CASE"], ["customers", "products", "shipments"], [
    ...
    Lesson("aggregates", 18, "Aggregates", ["COUNT", "SUM", "AVG"], ["products", "orders", "order_items"], [
    ...
    Lesson("grouping", 19, "GROUP BY and HAVING", ["GROUP BY", "HAVING"], ["products", "orders", "order_items", "customers"], [
    ...
    Lesson("conditional-aggregation", 20, "Conditional Aggregation", ["SUM(CASE)", "COUNT(CASE)"], ["orders", "order_items", "customers", "products"], [
    ...
    Lesson("union-intersect-except", 21, "UNION / INTERSECT / EXCEPT", ["UNION", "UNION ALL", "INTERSECT", "EXCEPT"], ["customers", "orders", "products", "order_items", "shipments"], [
    ...
    Lesson("set-operations-advanced", 22, "Set Operations", ["UNION", "INTERSECT", "EXCEPT"], ["customers", "orders", "products", "order_items"], [
    ...
    Lesson("advanced-review", 23, "Subqueries and Sets", ["subquery", "UNION", "EXCEPT"], ["products", "customers", "orders", "order_items"], [
    ...
    Lesson("ctes", 24, "Common Table Expressions", ["WITH", "readability"], ["customers", "orders", "order_items", "products"], [
    ...
    Lesson("recursive-ctes", 25, "Recursive CTEs", ["WITH RECURSIVE", "hierarchies"], ["employees", "customers"], [
    ...
    Lesson("window-ranking", 26, "Window Ranking", ["ROW_NUMBER", "RANK", "PARTITION BY"], ["products", "customers", "orders", "order_items"], [
    ...
    Lesson("window-analytics", 27, "Running Totals and Moving Averages", ["SUM OVER", "AVG OVER"], ["orders", "order_items", "products"], [
    ...
    Lesson("top-n-per-group", 28, "Top N Per Group", ["ROW_NUMBER", "filter ranked rows"], ["products", "orders", "order_items"], [
    ...
    Lesson("execution-order", 29, "Query Order", ["WHERE", "GROUP BY", "HAVING", "ORDER BY"], ["orders", "order_items", "customers"], [
    ...
    Lesson("indexes-query-plans", 30, "Indexes and Query Plans", ["CREATE INDEX", "EXPLAIN QUERY PLAN"], ["products", "orders", "customers", "order_items"], [
    ...
    Lesson("interview-filters", 31, "Interview Filtering Patterns", ["BETWEEN", "NOT IN", "multi-condition"], ["products", "customers", "orders"], [
    ...
    Lesson("insert", 32, "INSERT Rows", ["INSERT"], ["products", "customers", "orders", "order_items"], [
    ...
    Lesson("update", 33, "UPDATE Rows", ["UPDATE", "SET"], ["products", "orders", "support_tickets"], [
    ...
    Lesson("delete", 34, "DELETE Rows", ["DELETE"], ["products", "orders", "support_tickets"], [
    ...
    Lesson("create-table", 35, "CREATE TABLE", ["CREATE TABLE", "types"], ["products"], [
    ...
    Lesson("alter-table", 36, "ALTER TABLE", ["ALTER TABLE"], ["products", "customers"], [
    ...
    Lesson("drop-table", 37, "DROP TABLE", ["DROP TABLE", "IF EXISTS"], ["support_tickets", "shipments"], [
    ...
    Lesson("insert-select", 38, "CREATE AS & INSERT SELECT", ["CREATE TABLE AS", "INSERT INTO SELECT"], ["orders", "products"], [
    ...
    Lesson("leetcode-patterns", 39, "Classic Interview Patterns", ["second highest", "duplicates", "gaps"], ["products", "customers", "orders"], [
    ...
    Lesson("business-analytics", 40, "Business Intelligence SQL", ["JOIN", "arithmetic", "business-logic"], ["customers", "orders", "order_items", "products", "shipments"], [
    ...
    Lesson("final-interview-sets", 41, "Final Interview Sets", ["multi-step", "analytics", "business SQL"], ["customers", "orders", "order_items", "products", "shipments", "support_tickets"], [
